...
def main():
    ...
    with tab2:
        ...
        if 'parsed_stations' in st.session_state:
            ...
            with col2:
                # --- ✅ FIX: Trigger Backend Reload on Save ---
                if st.button("💾 Save to Disk & Reload", use_container_width=True, type="primary"):
                    try:
                        ...
                        if parl and assm:
                            if save_geography_data(parl, assm, data_to_save):
                                ...
                                try:
                                    requests.post(f"{API_URL}/geography/reload")
                                    st.toast("Backend Index Reloaded Successfully! 🚀")
                                except Exception as e:
                                    st.warning(f"File saved, but could not reload backend: {e}")
        ...
        # --- IMPROVED FILE MANAGEMENT SECTION (With Delete) ---
        st.divider()
        st.subheader("📁 Manage Geography Files")
        
        parliamentary_list = get_parliamentary_constituencies()
        if parliamentary_list:
            for parl in parliamentary_list:
                with st.expander(f"🏛️ {parl}", expanded=True):
                    assemblies = get_assembly_constituencies(parl)
                    if assemblies:
                        for assm in assemblies:
                            col_text, col_del = st.columns([4, 1])
                            
                            # Load data to show count
                            data = load_geography_data(parl, assm)
                            
                            with col_text:
                                st.markdown(f"**{assm}** ({len(data)} locations)")
                            
                            with col_del:
                                # 🗑️ DELETE BUTTON
                                if st.button("🗑️ Delete", key=f"del_{parl}_{assm}"):
                                    file_to_del = GEOGRAPHY_BASE_PATH / parl / f"{assm}.json"
                                    try:
                                        os.remove(file_to_del)
                                        st.success(f"Deleted {assm}.json")
                                        # Trigger backend reload
                                        try:
                                            requests.post(f"{API_URL}/geography/reload")
                                        except: pass
