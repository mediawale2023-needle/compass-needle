...
def _load_schemes():
    try:
        ...
        # Load corrected budget figures from enriched_pfms_data.json (generated by
        # scripts/build_enriched_schemes.py). This overlays verified BE values and
        # fixes data errors (copy-paste inflations, multi-year outlays, etc.) so that
        # budget_numeric — used for sorting and ministry totals — is accurate.
        enriched_by_id = {}
        try:
            import re as _re
            enriched_path = os.path.join(os.path.dirname(__file__), "data", "enriched_pfms_data.json")
            with open(enriched_path, "r", encoding="utf-8") as f:
                enriched_data = json.load(f)
            for e in enriched_data.get("schemes", []):
                if e.get("data_quality") not in ("NOT_A_SCHEME", "MISSING") and e.get("be_cr") is not None:
                    enriched_by_id[e["id"]] = e["be_cr"]
        except (FileNotFoundError, json.JSONDecodeError):
            pass

        for s in schemes:
            if s.get("id") in enriched_by_id:
                s["budget_numeric"] = enriched_by_id[s["id"]]
            else:
