...
def _dev_govt_dom_inspection_enabled() -> bool:
    """Temporary local-only Playwright DOM inspection bridge gate.

    Off by default and hard-disabled in production, even if the flag is set.
    """
    if os.getenv("ENV", "development").strip().lower() == "production":
        return False
    return os.getenv("GOVT_SYNC_DEV_DOM_INSPECTION", "").strip().lower() == "true"


@router.get("/dev/govt/session/{session_id}/dom")
async def dev_govt_live_session_dom(session_id: str, user=Depends(get_current_user)):
    """Development-only, read-only DOM snapshot for an existing live govt session.

    This endpoint is intentionally not a browser-control endpoint: it never
    clicks, types, navigates, uploads, submits, or executes caller-supplied JS.
    It exists only so local calibration can inspect the Playwright page that
    the dashboard is already streaming visually.
    """
    if not _dev_govt_dom_inspection_enabled():
        raise HTTPException(404, "Not found")

    tid = get_tenant_or_fail(user)
    from modules.govt_sync.browser_session import get_session_meta, inspect_live_session_dom, is_session_expired
    ...
    if is_session_expired(session_id):
        raise HTTPException(404, "Live session not found")
    if not meta.get("portal_name"):
        raise HTTPException(400, "Not a government portal session")

    snapshot = await inspect_live_session_dom(session_id)
    if not snapshot:
        raise HTTPException(404, "Live session not found")
...
def _get_tamil_nadu_live_status_context(case_id: int, session_id: str, user: dict):
    ...
    Mirrors _get_tamil_nadu_live_filing_context but inverts the filed check:
    a status read only makes sense once the case IS filed and has a reference
    number. Kept as its own helper so the filing path is not touched.
    """
    tid = get_tenant_or_fail(user)
    from modules.govt_sync.browser_session import get_live_session
...
async def govt_start_tn_status_check_session(case_id: int, user=Depends(get_current_user)):
    ...
    # Land staff on My Petitions after they log in, not the filing form. Done
