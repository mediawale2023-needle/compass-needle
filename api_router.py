...
# ─────────────────────────────────────────
# CITIZEN REPLY — primary account only; never changes complaint lifecycle
# ─────────────────────────────────────────
...
async def notify_citizen(case_id: int, request: Request, body: Optional[CitizenNotifyRequest] = None, user=Depends(get_current_user)):
    ...
    # Communication and resolution are independent staff actions. In particular,
    # do not modify status_changed_at, resolved_at, or government lifecycle here.
    try:
        _log_case_activity(tid, case_id, user.get("username", ""), "citizen_notified", new_value="whatsapp_sent")
    except Exception:
        pass  # nosec B110
    logger.info("Citizen reply sent: case=%s by=%s tid=%s", case_id, user.get("username"), tid)
