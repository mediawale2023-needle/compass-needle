...
Mock Playwright Page/Locator surface, asyncio.run, SQLite env. No real portal
is contacted. These tests pin the fail-closed contract — the adapter must
never return STATUS_CHECKED unless an authenticated page was reached and a
status was read and normalized.

This test file has no dependency on any Tamil Nadu filing/submission code —
modules/govt_sync/filing/ does not exist on this branch, and the status
adapter under test does not import from it.
"""
import asyncio
import os
import re
...
def test_no_filing_package_dependency():
    # Guardrail for this minimal read-only release: no filing package exists
    # on this branch, and the status module must not import it.
    import ast
    import importlib.util
    assert importlib.util.find_spec("modules.govt_sync.filing") is None
    import modules.govt_sync.status.tamil_nadu as tn_status_module
    source = ast.parse(open(tn_status_module.__file__).read())
    imported_modules = [
        node.module or "" for node in ast.walk(source) if isinstance(node, ast.ImportFrom)
    ]
    assert not any("filing" in m for m in imported_modules)
