...
- Date: 2026-09-19
- Request: Fix the MP Settings dead end where a tenant with no government portal contact number saw "Send OTP" fail with "No portal contact number on file for this tenant" and had no way to add one. Give the MP an option to set that number themselves (primary number only for now).
- Summary: Root cause confirmed against production — `tenants.govt_contact_primary_number` and `..._fallback_number` were both NULL for the affected tenant, so `_get_portal_contact_number()` returned None and `POST /api/govt/otp/send` 400'd. The only existing write path was admin-only (`PATCH /admin/mps/{id}/govt-contact`) and has no Admin UI, so the number was unreachable from the MP app entirely. Added MP-facing `PATCH /api/govt-portal/contact-number`: tenant identity derived solely from `get_tenant_or_fail` (no client-supplied tenant id exists on the request model), gated to the existing `_is_primary_workspace_user` roles (owner/mp/admin) rather than a new permission concept, validating/normalizing input to the bare 10-digit Indian mobile portals expect (`+91`, `0091`, leading zero, spaces/hyphens/brackets all accepted; invalid input rejected before any write so it can never clear a good number). `/api/govt-portal` now also returns `portal_contact_number`. Settings' Government Portal card shows the number on file, auto-opens the input when none is set, supports edit/cancel, disables "Send OTP" until a number exists, and shows non-primary staff a "ask the workspace lead" message. Only the primary number is MP-settable; the fallback stays admin-managed.
- Validation: new `tests/test_govt_portal_contact_number.py` `58 passed` (tenant isolation incl. body-tampering and a structural signature assertion, role authorization incl. parity with `_is_primary_workspace_user`, normalization/validation across realistic and hostile inputs, safe-update, `/api/govt-portal` read-back, OTP integration with the portal boundary mocked, masked-audit and no-plaintext-logging assertions); new `frontend/tests/settings-govt-portal-contact.test.jsx` `11 passed` (verified non-vacuous by reverting the Send-OTP guard and watching the test fail); full government-sync selection `446 passed`; full frontend suite `60 passed`; frontend production build clean.
- Files touched: `api_router.py`, `frontend/app/dashboard/settings/page.js`, `tests/test_govt_portal_contact_number.py` (new), `frontend/tests/settings-govt-portal-contact.test.jsx` (new), `TASK_LOG.md`.
- Risks or follow-ups: **Not deployed.** Audit rows record only a masked number (`******NNNN`) via the existing `_append_admin_audit_log` append-only log, reusing the same `govt_contact` target_type the admin-side equivalent writes — the full number is never logged, on success or failure. **Separate task for the Admin Dashboard redesign: `PATCH /admin/mps/{tenant_id}/govt-contact` has existed with no Admin UI at all, which is why this gap was only reachable from the MP side; the Admin surface needs a government-contact management view (and this is a concrete instance of the broader "operational capability exists but is not surfaced" finding).** Also noted for that audit: the `set-govt-contact-number` workflow takes the number as a plaintext dispatch input and prints it in its before/after SELECT, which on this public repo would publish it to a world-readable log — it should be changed to a secret-based input before any future use.
...
- Files touched: `api_router.py`, `modules/govt_sync/cookie_sessions.py`, `modules/govt_sync/adapters/tamil_nadu_http.py`, `tests/test_tamil_nadu_inboard_status.py`, `tests/test_govt_status_poll_orchestration_characterization.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`.
- Verification: focused TN/poller boundary tests 49/49; combined TN diagnostics, cookie-session, orchestration, normalization, import-cycle, and protected-state suite 314/314; full government/state selection 386/386; `py_compile` and `git diff --check` passed. No frontend/schema/poller/protected-adapter change, live portal request, push, merge, or deployment occurred. The unrelated `data/geography/Ghaziabad/Loni.json` modification remained untouched and unstaged.
...
- Summary: Added TN-only verification-mode state to `GovtSyncSection`. Filed TN cases now suppress the shared hosted-session manager, filing worksheet/reopen controls, and live browser unless staff explicitly clicks `Verify access`; discovery polling does not attach a session. Explicit verification can reconnect the existing same-case session or start the established status session. Successful promotion refreshes verified access, closes that exact attached live session, exits verification mode, and returns to compact `Government status` controls. Verified action-panel checks dispatch `/govt/poll` without scrolling; unverified checks reveal guidance but do not launch/reconnect a browser. Non-TN and unfiled filing workspaces preserve their previous rendering.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/tests/briefcase-tamil-nadu-session.test.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`.
- Verification: Focused TN lifecycle tests passed (15/15); full frontend tests passed (43/43); frontend production build passed; GovtSync regression bundle passed (282/282); Caddy validator passed (20/20). Protected Rajasthan/Karnataka/Maharashtra adapters, TN HTTP adapter, backend API, filing, OTP, browser-session infrastructure, and Caddy have empty diffs. Committed as `2f9f8ff501fe0f3b619dd152d57d905c89db3aa8`, pushed to `origin/main`, and deployed successfully by Vercel to `compass-needle-lacd` deployment `FwCtzqevRNCWkeCut2hz1MwG6C8k`. The production Briefcase chunk exposes the compact `Government status`/`Verify access` presentation plus `/govt/poll`, with browser start/promotion retained only for explicit verification. No live TN request or grievance mutation occurred.

- Date: 2026-09-09
- Request: Correct the Briefcase right-side `Check government status` CTA so Tamil Nadu normal status checks use the inboard HTTP poll path and never silently launch the live portal.
- Summary: Added a status-specific imperative action on `GovtSyncSection` and rewired the already-filed action-panel CTA from `handleEscalateClick` to `handleGovernmentStatusClick`. Verified TN cookie access routes to `POST /api/cases/{case_id}/govt/poll`; unverified TN access only sets verification guidance and does not call the live-session starter; the existing `Verify on Tamil Nadu portal` button remains explicitly wired to `handleStartTnStatusSession`. No backend, adapter, browser-session, filing, OTP, or Caddy behavior changed.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/tests/briefcase-tamil-nadu-session.test.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`.
- Verification: Focused TN frontend test passed (10/10); full frontend tests passed (38/38); frontend production build passed; established GovtSync regression bundle passed (282/282); Caddy routing validator passed (20/20). Impeccable detector reported one pre-existing side-border warning at modal line 1332, outside this behavior-only patch and intentionally untouched. Protected Rajasthan/Karnataka/Maharashtra adapter diffs and TN HTTP adapter/backend/Caddy diffs are empty. Committed as `0995e3aecb4bc422b9fef2053b9e1bc1925ae19a`, pushed to `origin/main`, and successfully deployed by Vercel to the `compass-needle-lacd` MP frontend; the deployed Briefcase chunk contains `checkGovernmentStatus`, `/govt/poll`, and the new no-silent-browser verification guidance. The AWS backend deploy correctly did not run because this commit contains no backend-path changes.

- Date: 2026-09-09
- Request: Deploy the validated Tamil Nadu GovtSync inboard HTTP status-checking implementation to production, while excluding unrelated `data/geography/Ghaziabad/Loni.json` work and avoiding any live TN grievance mutation.
- Summary: Created focused commit `cd74ff6d5f0b49aacb01158cd9a49c594dfae8dd` (`Add Tamil Nadu inboard HTTP status checks`) containing only the intended TN inboard status adapter, Fernet cookie-session persistence, status-session promotion endpoint/UI, Caddy route/validator update, DB table DDL/model, portal config, tests, and env example. Pushed `cd74ff6d` to `origin/main`, triggering GitHub Actions production deploy run `34321130794` / job `102367791801`; the workflow SSH deploy step explicitly ran `git reset --hard cd74ff6d5f0b49aacb01158cd9a49c594dfae8dd`, rebuilt/recreated `backend`, `backend_govt_live`, and `caddy`, and the workflow health step passed. Local post-deploy Caddy validator had already passed 20/20 before push and the deployed workflow installed the same committed Caddyfile. Public `https://api.theneedle.in/health` returned healthy after deployment. No live TN portal/status request was made from Codex because no authenticated Needle/TN production session or production private key was available; SSH verification from Codex was blocked by missing private key, and SSM was denied by IAM.
- Files touched: No code files changed after the deployment commit. This local `TASK_LOG.md` entry is uncommitted post-push bookkeeping per repo protocol.
- Risks or follow-ups: Follow-up hardening remains: remove frontend exposure/dependency on live-browser `session_id`/`ws_path` and make Tamil Nadu session promotion case-scoped with backend-side session resolution. Production `GOVT_COOKIE_SESSION_KEY` presence could not be independently verified from Codex because direct SSH lacked the production private key and SSM access is denied; if absent, TN cookie promotion/status fails closed and staff must configure the key before using TN inboard status checks. The unrelated `data/geography/Ghaziabad/Loni.json` change stayed unstaged/uncommitted and was not deployed.

- Date: 2026-08-31
- Request: Productionize the Overview page — continue from Codex's partial `api_router.py` work (do not discard), factor the Briefcase grouping helper so Overview can reuse grouped thread bundles, keep `/api/cases` unchanged, compute Overview metrics at CASE/THREAD level, then wire the LOCKED Overview prototype design into the real `/dashboard` route against a real `GET /api/dashboard/overview` contract. No push, no deploy.
- Summary: Backend — factored `_group_briefcase_cases` into `_briefcase_group_bundles(cases, sort)` (returns `{anchor, members_by_received, members}` bundles); `_group_briefcase_cases` is now a one-line wrapper (`[b["anchor"] for b in ...]`) so `/api/cases` output is byte-identical (verified: `test_briefcase_api.py`+`test_case_buckets_api.py` show identical pass/fail before/after — the 5 failures are pre-existing SQLite `DISTINCT ON` limitations on `main`). Completed Codex's `GET /api/dashboard/overview`: fixed `_dashboard_time_ago` tz bug (naive `_utcnow()` vs tz-aware `Z` ISO), reordered `attention_counts` to precedence order (needs_review → needs_location → sync_issues → govt_updates → ready_to_file), enriched `attention_queue` rows with explicit `thread_count`/`channel`/`needle_status`/`govt_status`/`assigned_to`/`recency`, added `is_deleted` guards to the letterbox counts, made `government_tracking.issues` carry `<case_ref> · <state>`. Attention buckets are mutually exclusive at thread level (if/elif on grouped bundle). Govt Updates predicate is conservative: latest persisted `govt_submission_log` `action=status_polled` with `payload.changed=true` (transient — clears on next poll; no seen/unseen model). Frontend — added `components/dashboard/OverviewDashboard.jsx` (the locked prototype design, parameterized on a `data` prop; removed dev-only chrome, dropped the fabricated hotspot `x/y` dots and the `issuePressure.note` prototype copy, added row-click → Case Detail + Next-Action nav + external Local Signals links). Rewrote `hooks/useDashboardOverview.js` to fetch `/api/dashboard/overview` + `/api/dashboard/engagements` + `/api/news?news_type=local` + `/api/maps/seat-manifest`. Added `mapOverviewResponse`/`getOverviewIsEmpty` to `lib/dashboard-mappers.js`. `app/dashboard/page.js` is now a thin wrapper rendering `<OverviewDashboard>`; old KPI/donut/SLA components are orphaned (left in place, not deleted). Rewrote `tests/dashboard.test.jsx` for the new design. Follow-up layout-flow fix (no card/type/color/data changes): replaced the paired-row desktop grid with two independent vertical columns beneath the attention strip — LEFT = Attention Queue → Issue Pressure → Office Pending, RIGHT = Government Tracking → Constituency Pressure → Today → Recent Movement — each with its own flow so Issue Pressure starts right after the queue instead of waiting for the rail; Local Signals is full-width beneath whichever column is taller. Below 1440px (where the fixed 220px app rail no longer leaves room for the ~65/35 split) the two wrappers flatten (`display:contents`) and re-flow via `order`+`flex-basis` into the approved responsive stack: Attention Queue full width → Government Tracking + Constituency Pressure paired (they drop to full-width stacked at <=1023) → Issue Pressure / Today / Office Pending / Recent Movement each full width → Local Signals full width. Every card uses the full workspace width at 1280/1024 (no narrow partial-width cards); 1440 and 390 layouts unchanged. `npm run build` green, `vitest` 4/4 green, Playwright screenshots captured at 1440/1280/1024/390 (no page overflow, no console errors; measured card widths confirm full-width usage and the paired Govt/Constituency row at 1280 & 1024).
- Files touched: `api_router.py`, `frontend/app/dashboard/page.js`, `frontend/hooks/useDashboardOverview.js`, `frontend/lib/dashboard-mappers.js`, `frontend/components/dashboard/OverviewDashboard.jsx` (new), `frontend/tests/dashboard.test.jsx`, `TASK_LOG.md`
- Risks or follow-ups: `_dashboard_govt_sync_map` and `get_cases` both use Postgres `DISTINCT ON`, so the Overview endpoint can only be integration-tested on Postgres (matches existing repo constraint). `constituency_pressure`/`issue_pressure` count all tenant threads by location (not only unresolved). Layout-flow fix: the independent two-column composition engages only at >=1440px (below that the fixed 220px app rail leaves too little room for the intended ~65/35 split); 1280–1439 and 1024–1279 both use the approved responsive stack (full-width queue, Govt+Constituency paired down to 1024, everything else full width). Visual review approved at 1440/1280/1024/390; the dev-only `frontend/app/dashboard/overview-prototype/` route was removed as part of this commit. The unrelated in-tree `api_router.py` govt-sync comment reword (GOVERNMENT DEPARTMENT SYNC block) was deliberately left unstaged/uncommitted. Committed but not pushed, not deployed.

- Date: 2026-08-31
- Request: Make one final visual-polish pass on the isolated Overview prototype only, preserving the approved IA/layout and focusing exclusively on contrast/readability, restrained next-action affordance contrast, status label legibility, mobile message/metadata separation, then capture only 1440 and 390 screenshots, run the build, and stop before productionization.
- Summary: Tuned only `frontend/app/dashboard/overview-prototype/page.js` visual hierarchy: strengthened the established Needle ink hierarchy for raw citizen messages, section headings, case IDs, issue titles, operational numbers, action text/borders, and status-label text while keeping secondary/meta details muted and leaving the cream paper background unchanged. Preserved the approved desktop grid, responsive breakpoints, module order, mock data structure, mobile four-card queue, and prototype-only labels. Captured updated 1440 and 390 review screenshots into `.impeccable/review/`.
- Files touched: `frontend/app/dashboard/overview-prototype/page.js`, `.impeccable/review/overview-prototype-1440.png`, `.impeccable/review/overview-prototype-390.png`, `TASK_LOG.md`
- Risks or follow-ups: Prototype remains dev-only with mock data and no production Overview/API changes. Visual approval is still required before any production implementation.

- Date: 2026-08-31
- Request: Make one visual-polish pass on the isolated Overview prototype only, preserving approved IA/aesthetic, fixing desktop first-row dead space, queue message hierarchy/actions/state semantics, government tracking journey, map density, and mobile 390 queue/counter efficiency, then recapture 1440/1280/1024/390 screenshots and stop before productionization.
- Summary: Refined `frontend/app/dashboard/overview-prototype/page.js` without touching production Overview or other modules. Tightened the desktop first layout region, strengthened raw citizen-message hierarchy, converted next actions into restrained compact buttons, added subtle semantic state-badge families, made Government Tracking read as a connected journey while keeping resolved quiet and sync issues prominent, enlarged/tightened the constituency map artwork, preserved prototype annotations, kept 1280/1024 queue-first responsive behavior, and converted the 390 queue into four compact operational cards plus `View all cases ->`. Recaptured the four review screenshots into `.impeccable/review/`.
- Files touched: `frontend/app/dashboard/overview-prototype/page.js`, `.impeccable/review/overview-prototype-1440.png`, `.impeccable/review/overview-prototype-1280.png`, `.impeccable/review/overview-prototype-1024.png`, `.impeccable/review/overview-prototype-390.png`, `TASK_LOG.md`
- Risks or follow-ups: Still prototype-only mock data with no live API contract. Production Overview remains intentionally untouched; wait for visual approval before implementation.

- Date: 2026-08-31
- Request: Build an isolated, dev-only MP Overview prototype using Compass Needle's existing visual language, clearly marked mock data, and capture 1440/1280/1024/390 screenshots before any production implementation.
- Summary: Added `frontend/app/dashboard/overview-prototype/page.js` as a standalone prototype route under the existing dashboard shell. The page uses local mock data marked as prototype-only and keeps the production Overview route untouched. The layout prioritizes a dense Attention Queue, smaller Government Tracking and Constituency Pressure rail, secondary Issue Pressure/Today/Office Pending/Recent Movement sections, and a thin Local Signals strip. Captured full-page acceptance screenshots at 1440, 1280, 1024, and 390 into `.impeccable/review/`.
- Files touched: `frontend/app/dashboard/overview-prototype/page.js`, `.impeccable/review/overview-prototype-1440.png`, `.impeccable/review/overview-prototype-1280.png`, `.impeccable/review/overview-prototype-1024.png`, `.impeccable/review/overview-prototype-390.png`, `TASK_LOG.md`
- Risks or follow-ups: Prototype uses illustrative mock values only and has no live API contract. Stop point is visual review; production Overview implementation remains intentionally untouched and should not begin until approved.

- Date: 2026-08-24
- Request: Implement the Karnataka iPGRS interactive status-check adapter against a live, legitimately-filed grievance (real Grievance ID/mobile, CAPTCHA solved by the user in-browser, never OCR'd/bypassed), on top of the already-merged InteractiveStatusCheckMixin architecture — with an explicit design-review-then-approve step (4 named corrections given before implementation began) and no deploy without a further explicit go-ahead.
- Summary: Fetched Karnataka's real status-check page directly (public, read-only, no CAPTCHA/grievance data involved) and read its inline, un-obfuscated JavaScript, which fully CONFIRMED the request/response contract that an earlier evidence report had only partially inferred — exact field names (`GrievanceId`/`MobileOrEmail`/`Captcha`, form-urlencoded, not JSON), the exact response envelope (`{success, message, data: {..., Status}}`), and confirmed no CSRF token is sent by the real client for this call. Built `modules/govt_sync/adapters/karnataka_ipgrs.py`: `KarnatakaAPIAdapter(InteractiveStatusCheckMixin, ManualAssistedAdapter)` — `start()` opens a `requests.Session()`, loads the real status page then the CAPTCHA image (establishing the session cookie the same way a browser does), returns the CAPTCHA as a base64 data URI; `advance()` replays the same session's cookies, POSTs the confirmed field set, parses only `Status` (never Department/Category/Description/PendencyDetails — deliberately not extracted, per explicit instruction) through the existing, unmodified `normalize_status_keywords()` — "Registered & Sent for Scrutiny" normalizes to "submitted". Attempts live in a process-local in-memory dict (same precedent as `browser_session.py`'s `_sessions`), explicitly not `govt_otp_sessions`, no DB table, no Redis, documented as not surviving a backend restart. `mobile_or_email` comes from the existing `_get_portal_contact_number(tid)` — never staff-typed. Two small, justified additions to the already-merged `status_flow.py` (flagged for explicit approval before building, and corrected per the 4 named instructions): `TransportKind.AJAX_FORM` (the real encoding, not `AJAX_JSON`) and `HumanVerificationRequirement.challenge: str | None` (generic, holds the CAPTCHA image for Karnataka). Two new endpoints (`/cases/{id}/govt/status-check/start` + `/advance`) and one new `interactive_status_check` flag on `GET /govt-portal` — all additive, `govt_poll_case` and every existing endpoint left byte-identical. Minimal CAPTCHA UI added to `BriefcaseCaseModal.jsx`'s existing Check Status flow (image + text input + Verify/Cancel), reusing existing visual conventions, routed only when `interactive_status_check` is true. Security discipline followed throughout: CAPTCHA text, CAPTCHA image/base64, session cookies, and portal contact number are never logged anywhere, under any code path; cookies never appear in any API response. 15 new tests (`tests/test_karnataka_status_check.py`) assert the exact confirmed request field names, cookie continuity between `start()`/`advance()`, exact response parsing, the "submitted" normalization outcome, `checked=True`, `supports_unattended_status_check=False`, MRO, registration/dispatch, a full endpoint round-trip, and — critically — that the poller never calls `check_status`/`start`/`advance` on Karnataka (`assert_not_called()`) while still calling it on Rajasthan (regression guard in the other direction, since a naive skip-check bug could silently exclude the wrong portal). `tests/test_govt_duplicate_filing.py` (12/12) and the full local-Postgres Rajasthan OTP validation script both re-run unchanged and green; `git diff` on `base.py`/`rajasthan_sampark.py`/`otp_sessions.py`/`poller.py`/`manual.py` confirmed empty.
- Files touched: `modules/govt_sync/adapters/karnataka_ipgrs.py` (new), `tests/test_karnataka_status_check.py` (new), `modules/govt_sync/adapters/status_flow.py`, `modules/govt_sync/adapters/__init__.py`, `modules/data/govt_portals.json`, `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: True end-to-end verification against the live portal (a real CAPTCHA solved by staff through Needle's new UI, not just a mocked request/response) has not been done from this environment and requires a human — first live staff use will be the true test, same caveat as Rajasthan's OTP flow when it first shipped. An in-flight attempt (CAPTCHA shown, not yet answered) does not survive a backend restart by design — staff would need to click "Check status" again; this is an accepted, documented limitation, not a bug. Karnataka's `__RequestVerificationToken` CSRF-gap-on-a-read-endpoint observation (from the earlier evidence-gathering session) remains queued for the same eventual responsible-disclosure batch as the two Rajasthan Sampark findings — not actioned. Maharashtra (2-stage, CAPTCHA+OTP+CAPTCHA, `FRESH_THIS_FLOW` session threading via `cid`) is the next planned adapter, intentionally not started — Karnataka was built first specifically because it is the simpler, single-stage case, per the plan agreed earlier in this engagement.

- Date: 2026-08-24
- Request: Live-tested Karnataka (iPGRS) and Maharashtra (Aaple Sarkar) grievance status lookups against real, legitimately-filed grievances (own investigation, CAPTCHA/OTP entered by the user, never solved/bypassed by Claude), then asked for the resulting evidence to be treated as an authoritative specification: redesign the GovtPortalAdapter status-check architecture to represent a status check as a sequence of stages, without implementing Karnataka/Maharashtra or touching Rajasthan, with explicit sign-off required before any code was written.
- Summary: Two design-review passes (current-architecture inspection → gap analysis → proposed hierarchy → per-portal lifecycle walkthrough, twice, refined the second time specifically to correct an early overreach — the first pass didn't yet resolve how a synchronous check_status() call could represent a paused, human-in-the-loop flow) preceded any code. Findings: Karnataka is a 1-stage flow (grievance ID + mobile/email + one CAPTCHA, AJAX/JSON). Maharashtra is a genuine 2-stage flow — mobile/email + CAPTCHA triggers a real SMS OTP, which unlocks a server-issued `cid` required by stage 2 (token + a second, distinct CAPTCHA) — proving a later stage can depend on an earlier stage's server-supplied output, which the existing model had no way to represent. Shipped the minimal additive fix, once approved: a new `modules/govt_sync/adapters/status_flow.py` (purely descriptive `StatusCheckStage`/`StatusCheckFlow` types — inputs/produced_values/human_verification/transport/session_requirement per stage — plus `StatusCheckAttempt` runtime state and `InteractiveStatusCheckMixin`, which provides an honest `check_status()` stub, i.e. `checked=False`, rather than pretending an interactive lookup completed synchronously); one new `GovtPortalAdapter.supports_unattended_status_check: bool = True` class attribute (base.py); one new skip-check in poller.py's loop so interactive portals are structurally never attempted by the unattended background poller (no human present to solve a CAPTCHA), rather than caught-and-skipped via an exception. `describe_flow()`/`start()`/`advance()` are deliberately left `NotImplementedError` — no concrete adapter uses this mixin yet, so there's nothing real to generalize a body from. Explicitly kept `OtpGatedStatusMixin` (Rajasthan's shape — verify once out of band, cached session reused for many free automated checks) and `InteractiveStatusCheckMixin` (Karnataka/Maharashtra's shape — every single lookup needs a live human, at call time, with no evidence yet that verification is reusable across lookups) as two separate, never-merged mechanisms, per explicit instruction. Recorded the full reference architecture — the 3-axis taxonomy (Execution/Authentication/Human verification), the independence rules, the confirmed-vs-Unknown classification per portal, the "Unknown is a valid state" principle, and the 10-step next-state investigation procedure — in `modules/govt_sync/adapters/__init__.py`'s module docstring and `PROJECT_MEMORY.md`, all before this session's code changes (a separate, earlier sign-off in the same conversation). Re-ran the full local-Postgres end-to-end validation (schema, real seed, MRO, `get_adapter()` dispatch, the complete Rajasthan OTP state machine, both `/api/govt/otp/*` endpoints called directly) after every change in this entry — zero regressions, all passing — plus `tests/test_govt_duplicate_filing.py` (12/12).
- Files touched: `modules/govt_sync/adapters/status_flow.py` (new), `modules/govt_sync/adapters/base.py`, `modules/govt_sync/poller.py`, `modules/govt_sync/adapters/__init__.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Purely additive — no existing portal's behavior, `StatusResult` shape, `check_status()` signature, `OtpGatedStatusMixin`, `otp_sessions.py`, `rajasthan_sampark.py`, or any existing API endpoint changed. Karnataka and Maharashtra are NOT implemented — `InteractiveStatusCheckMixin` has no concrete adapter using it yet. Building either requires, and was explicitly deferred to, a Karnataka-implementation-time design pass for: a persistence layer for `StatusCheckAttempt` between HTTP requests (a new table, analogous to but explicitly not reusing `govt_otp_sessions` — different reuse semantics), new staff-facing endpoints shaped like `/govt/otp/send`+`/verify` but generalized to "start"/"advance" a flow, and a frontend component to display a CAPTCHA image and collect the answer (nothing like this exists today — Rajasthan's OTP UI is plain text entry, no image). Per the evidence report's own explicit recommendation and this session's plan: implement Karnataka first (simpler, single-stage, stress-tests less) before Maharashtra (multi-stage, stress-tests the `FRESH_THIS_FLOW` session/produced_values threading) — not started. Karnataka's `__RequestVerificationToken` CSRF-gap-on-a-read-endpoint observation is a new, separate candidate for the same eventual responsible-disclosure batch as the two Rajasthan Sampark findings — not actioned.

- Date: 2026-08-22
- Request: "Extract the reusable government-sync/authentication framework from the Rajasthan implementation, then implement each state as a portal-specific adapter" — after a "how do we do this for other states" discussion found that of the 11 configured states, only Rajasthan/UP Jansunwai/CPGRAMS even attempt an automated status check today, and that UP's real tracker page (jansunwai.up.nic.in/ComplaintTracker, found after the user corrected a stale URL) adds a CAPTCHA Rajasthan's real API never had — a materially different, not-yet-solved problem, so no fabricated state adapters were built without real reverse-engineered API knowledge.
- Summary: Extracted the mobile-scoped-OTP session machinery that was living inside rajasthan_sampark.py into two reusable, portal-agnostic pieces: `modules/govt_sync/otp_sessions.py` (plain govt_otp_sessions DB layer — get/upsert/mark-failed/touch-used/verification_state, no portal knowledge) and `OtpGatedStatusMixin` in `modules/govt_sync/adapters/base.py` (the orchestration — start_verification/complete_verification/verification_state/check_status as instance methods, calling three abstract hooks a concrete adapter must implement: `_send_otp`/`_validate_otp`/`_fetch_status`). `check_status()` now accepts either a single `raw_status_text` or `status_text`+`sub_status_text` from `_fetch_status()`, whichever shape fits a given portal's real response — Rajasthan's `check_grievance_detail()` still returns the split form, validated both forms work. `rajasthan_sampark.py` shrank to just its portal-specific pieces: the gateway envelope caller, the three raw Sampark API functions, PII-safe field extraction, and a 6-line adapter class binding those three functions to the mixin's hooks (`class RajasthanSamparkAPIAdapter(OtpGatedStatusMixin, ManualAssistedAdapter)` — mixin listed first so its check_status() wins over ManualAssistedAdapter's HTML-scrape version in MRO; prepare_submission() still resolves to ManualAssistedAdapter unchanged, confirmed via `__mro__`/`__qualname__` inspection, not just assumed). api_router.py's `/govt/otp/send`, `/govt/otp/verify`, and `_govt_otp_verification_state` no longer hardcode an import of rajasthan_sampark.py — all three now dispatch through `get_adapter(portal)` + `hasattr()` checks, so a second OTP-gated state needs zero changes to api_router.py, only a new adapter module + one `_STATUS_CHECK_ADAPTERS` registration. `modules/govt_sync/adapters/__init__.py`'s module docstring now spells out the 3-step process for adding a new state's real adapter (trace real traffic first — never assume a portal fits the mixin; implement the three hooks if it's mobile-scoped-OTP-gated like Rajasthan; implement check_status() directly instead if it's gated some other way, e.g. UP Jansunwai's CAPTCHA, which does NOT fit this mixin and was correctly not force-fit into it). Re-validated the full flow end-to-end against real local Postgres after the refactor (not just py_compile): schema creation, the real seed path, MRO correctness, get_adapter() dispatch, the full OTP state machine via the new instance methods, both check_status() field-shape contracts, portal_id/id key aliasing, and both API endpoints called directly (success + wrong-OTP 400) — all passing, plus the pre-existing tests/test_govt_duplicate_filing.py suite (12 tests, untouched by this refactor) still green.
- Files touched: `modules/govt_sync/otp_sessions.py` (new), `modules/govt_sync/adapters/base.py`, `modules/govt_sync/adapters/rajasthan_sampark.py`, `modules/govt_sync/adapters/__init__.py`, `api_router.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: This is a pure internal refactor — no API contract, DB schema, or frontend behavior changed; Settings' Government Portal card and Briefcase's needs_verification shortcut (shipped earlier the same day) work unmodified. UP Jansunwai's real tracker was found to require reference number + mobile + CAPTCHA + OTP (`jansunwai.up.nic.in/ComplaintTracker`) — whether the CAPTCHA is enforced server-side (making automation materially harder than Rajasthan, which had none) is still unknown and needs the same real-network-trace investigation the user did for Rajasthan before any UP-specific adapter gets built; not started. The stale UP URL in modules/data/govt_portals.json (`samadhan.gov.in/Grievance/GrievanceStatus.aspx`, 404) was NOT corrected in this change — only the framework was extracted; updating the seed data to the real URL is a separate, still-open follow-up. No other state has been investigated for a real backend API at all.

- Date: 2026-08-22
- Request: Fix Rajasthan Sampark status checking (found broken via a real, user-filed reference number returning "Portal check inconclusive") by integrating Rajasthan Sampark's real backend gateway API, discovered via user-provided reverse-engineering research, replacing the previously-scrape-based status check. Plan first, build only after explicit confirmation of 4 decisions (status storage, OTP UI location, vulnerability disclosure timing, poll interval), then "build both phases."
- Summary: Rajasthan Sampark's status page turned out to be a client-side-rendered React SPA — plain `requests.get()` could never see real status data for any reference number, confirmed against a real citizen-filed grievance. Replaced it with a dedicated adapter (`modules/govt_sync/adapters/rajasthan_sampark.py`) that calls Sampark's real backend gateway (`POST /gateway/api/GatewayService/GetList`) directly. New `govt_portals.status_check_adapter` column lets `get_adapter()` dispatch to a portal-specific adapter before falling back to the generic scrape-based `ManualAssistedAdapter`, so this generalizes to any future portal with the same problem. Because Sampark's OTP verification is scoped to the mobile number (not per-grievance, valid ~28+ min across multiple grievances), added a new `govt_otp_sessions` table caching one verified session per `(tenant_id, portal_id)` — staff verify once via a new "Government Portal" card in Settings (`POST /api/govt/otp/send` + `/verify`), and the adapter/poller both silently reuse that cached session for every pending case on the portal until it goes stale, at which point `check_status`/the poller return `needs_verification` instead of a generic "inconclusive" (poller batches this per tenant+portal per run instead of re-failing every case). Briefcase's case-view "Check status now" surfaces a `Verify under Settings → Government Portal` shortcut inline when that happens. Adapter deliberately extracts only status/department/subject/dates from the gateway's ~130-field response — never the complainant PII block it also returns. Two real unauthenticated Sampark endpoints found during the investigation are unused by this integration and earmarked for separate responsible disclosure to RISL (not actioned yet, by design). Validated end-to-end against real local Postgres (not just py_compile) — schema creation via `Base.metadata.create_all` plus the one migration-only unique index, the real `seed_govt_portals()` seeding path, the full send-OTP/verify/expire/re-verify state machine, the adapter's `check_status` happy path and both failure modes, the `portal_id`/`id` key-aliasing across different SELECT call sites, and the two new `/api/govt/otp/send` + `/api/govt/otp/verify` endpoint functions called directly (success, wrong-OTP 400).
- Files touched: `sansadx_backend/db.py`, `main.py`, `modules/govt_sync/adapters/base.py`, `modules/govt_sync/adapters/manual.py`, `modules/govt_sync/adapters/rajasthan_sampark.py` (new), `modules/govt_sync/adapters/__init__.py`, `modules/govt_sync/poller.py`, `modules/govt_sync/seed.py`, `modules/data/govt_portals.json`, `api_router.py`, `frontend/app/dashboard/settings/page.js`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: True expiry of the mobile-level OTP verification beyond ~28 minutes is unknown (untested past that window) — if it turns out shorter in practice, staff will just see `needs_verification` more often, not a silent failure. End-to-end verification against the live Sampark API with a real OTP has not been done from this environment (requires a human to receive and enter a real SMS code) — first live use will be the true end-to-end test. Responsible disclosure of the two unauthenticated Sampark endpoints to RISL is confirmed-but-not-started, deliberately kept separate from this build per the user's own framing. Not yet deployed to EC2/production as of this entry.

- Date: 2026-08-20
- Request: Populate 100 mock complaints for Tenant #11 (Dubey, Udaipur Rural, Rajasthan) in their dashboard.
- Summary: Added `scripts/seed_mock_complaints.py` — dry-run by default (`--apply` to write), scatters N realistic-looking complaints across the tenant's real shared seat geography if any exists in `tenant_overrides` (falls back to a generic ward-name list otherwise), varied categories/subdomains from Needle's canonical taxonomy, weighted new/in_progress/resolved status mix, and `created_at` spread over the last 75 days so the dashboard shows an organic-looking backlog instead of 100 rows all timestamped the same second. Does not touch tenant/profile/user identity — only inserts `cases` rows for an existing tenant. Idempotent: every row carries `case_metadata.mock_seed=true` and a `case_ref` prefixed `MOCK-<tenant_id>-`; re-running with `--apply` deletes prior mock rows for that tenant first (`--preserve-existing` skips that).
- Files touched: `scripts/seed_mock_complaints.py`, `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Risks or follow-ups: Validated end-to-end against a real local Postgres instance (not just SQLite) before running on production — caught two real bugs this way: (1) `cases.assembly_constituency` is not an actual DB column (only ever written into `case_metadata` JSON throughout `main.py`), so the raw INSERT would have failed the same way `scripts/seed_contact_thread_demo.py`'s equivalent INSERT likely would if it's ever actually run against current production schema — that script has the identical latent bug and should get the same fix; (2) confirmed the Postgres-only `case_metadata::json->>'mock_seed'` delete-guard syntax actually works (SQLite can't test this at all, so it was invisible to a SQLite-only test).

- Date: 2026-08-20
- Request: Set up the 7 reachability-confirmed states (Bihar, Madhya Pradesh, Mizoram, Odisha, Punjab, Tamil Nadu, Tripura) now that reachability from EC2 was checked for all of them.
- Summary: Mapped `department_taxonomy` for all 7 (previously `{}`, blocking Escalate with "doesn't have its department list configured yet"), same best-effort treatment as Rajasthan/UP/Karnataka/Maharashtra. Researched via Wikipedia "Government of X" pages; each `source_note` documents exactly which department names were directly confirmed vs. inferred from standard Indian-state naming conventions. Odisha and Punjab came back fully confirmed; Bihar/Madhya Pradesh/Tamil Nadu/Mizoram have one or two standard-convention names (mainly the water-supply/PHE department, not always broken out separately in a general cabinet listing). Tripura is explicitly flagged lower-confidence — the source used didn't confirm School Education, Revenue, Women & Child Development, or Urban Development as separate departments at all, so 3 of its 9 mapped values are inferred, not confirmed; it should be the next one re-verified against a real login if this state gets a live tenant. `field_schema.taxonomy_verified` stays `false` for all 7 — none confirmed against a real post-login department dropdown yet. Uttarakhand intentionally excluded — its TLS handshake issue (see PROJECT_MEMORY.md) is still unresolved, not just untaxonomized.
- Files touched: `modules/data/govt_portals.json`, `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Risks or follow-ups: Needs a backend deploy (seed upserts `department_taxonomy`/`source_note` on every boot). Tripura's taxonomy is the least trustworthy of the 8 states now configured — prioritize it for real verification before assuming it's correct. `live_session_supported` was left unset (defaults `true`) for all 7 since reachability was already confirmed for each via `diagnose-portal-reachability.yml` before this pass.

- Date: 2026-08-19
- Request: Keep Maharashtra manual for now; Escalate should open grievances.maharashtra.gov.in in a new browser tab and show the worksheet in Needle, instead of attempting a live session that will just time out.
- Summary: Added `govt_portals.live_session_supported` (default true), set to `false` for Maharashtra only after confirming (SSH from EC2, both host and container network namespace) that TCP connections to grievances.maharashtra.gov.in time out on ports 80 and 443 — DNS resolves, but the handshake never completes; the same host connects instantly from outside EC2. `GET /api/govt-portal` now returns `live_session_supported` and a computed `entry_url` per portal. `POST /govt/session/start` 403s server-side for a `live_session_supported=false` portal regardless of the global `GOVT_LIVE_AUTOMATION_ENABLED` switch (this is a portal-level fact, not an ops toggle). Frontend: Escalate for such a portal now opens `entry_url` in a new browser tab (the staff's own network, not EC2's) and calls the worksheet-only path so department/subject/description still show in Needle to copy from; a small "Reopen {portal}" link appears afterward in case staff closes that tab. `window.open` is called before any `await` so it isn't blocked as an unrequested popup.
- Files touched: `sansadx_backend/db.py`, `main.py`, `modules/govt_sync/seed.py`, `modules/data/govt_portals.json`, `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `tests/test_govt_duplicate_filing.py`, `.github/workflows/diagnose-portal-reachability.yml` (new), `.github/workflows/diagnose-govt-sync.yml` (generalized), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Risks or follow-ups: Needs a backend deploy (migration + reseed run on startup). The other 8 states with empty `department_taxonomy` (Bihar, Madhya Pradesh, Mizoram, Odisha, Punjab, Tamil Nadu, Tripura, Uttarakhand) still need both the taxonomy mapping AND a reachability check like this one before assuming their live sessions would actually work — do that check before spending research time on taxonomy for a portal that might also be network-blocked.
...
- Files touched: `modules/govt_sync/browser_session.py`, `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `tests/test_govt_duplicate_filing.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: The IP-concentration/fingerprinting risk this same file's earlier entry describes still applies to the open+login+navigate step (unaffected by dropping autofill) — the emergency flag is the mitigation, not a fix. Filer-identity copy and AI-description PII-scrub questions remain open and out of scope for this patch.
...
- Risks or follow-ups: This records who/when/what for geography and reusable alias changes, but it still does not capture an operator-entered freeform “why” note. If ops later needs formal change rationale, add an explicit reason field to the Shared Geography save flows instead of inferring intent from the diff alone.
...
- Risks or follow-ups: This deploy intentionally excludes unrelated local edits in `data/geography/Ghaziabad/Loni.json` and `tenant_overrides.json`. After the backend deploy completes, live verification should focus on tenant 10 Briefcase behavior: grouped row count, `+N` badge, independent complaint switching in the drawer, and per-complaint resolve/status actions.

- Date: 2026-06-09
- Request: Reseed tenant 10 so the Briefcase thread demo uses real sibling complaint rows instead of legacy metadata-only child items.
- Summary: Rewrote `scripts/seed_contact_thread_demo.py` to generate 18 independent demo `cases` rows across three shared `contact_thread_id` groups (`3`, `6`, and `9` siblings with one suppressed spam message), removing the old `contact_thread_items`-based seed shape. Verified locally that the script now produces zero legacy pending-thread items, then copied the updated script into the live EC2 backend container with `docker cp` and reran it for tenant `10` / `Jadhav`. Direct Postgres checks on production now confirm `18` seeded rows, grouped under the expected three thread ids, with `0` demo rows still carrying `contact_thread_items`. A follow-up API check showed production backend code is still on the older Briefcase detail contract and not yet returning `thread_cases`, so the live data is ready but the independent thread-case UI still requires backend deploy of the newer Briefcase API code.
- Files touched: `scripts/seed_contact_thread_demo.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: The reseed itself is complete, but the user will not see the intended “independent complaints inside one thread” behavior on production until the current `api_router.py` / `frontend` Briefcase thread changes are actually deployed. Also note that ad-hoc EC2 script updates must be copied into `ec2-backend-1`; updating only `/opt/compass-needle/app` on the host does not change what `docker exec` runs.
...
- Date: 2026-06-08
- Request: Push the aspirant charcoal-gray palette update to GitHub for deployment.
- Summary: Pushed commit `d78e8a90` (`Set aspirant palette to charcoal gray`) to `origin/main`, publishing the aspirant color update so the backend auth/session theme payload and the shared sidebar theme helper now use `#2A2A2A` for aspirant MP/MLA accounts.
- Files touched: `TASK_LOG.md`
...
- Date: 2026-06-08
- Request: Push the aspirant olive palette update to GitHub for deployment.
- Summary: Pushed commit `6be67b6b` (`Set aspirant palette to olive`) to `origin/main`, publishing the aspirant color update so the backend auth/session theme payload and the shared sidebar theme helper now use `#272B07` for aspirant MP/MLA accounts.
- Files touched: `TASK_LOG.md`
- Risks or follow-ups: Unrelated local edits in `data/geography/Ghaziabad/Loni.json` and `tenant_overrides.json` remained intentionally unpushed. Users may need to sign out and back in before all stored-session surfaces reflect the refreshed aspirant theme color.

- Date: 2026-06-08
- Request: Change the aspirant MP frontend palette from warm brown to golden `#DAB433` and push it.
- Summary: Updated both the backend auth/session theme payload and the shared sidebar theme helper so aspirant MP/MLA accounts now use `#DAB433` instead of `#A27246`, while elected Lok Sabha/MLA and Rajya Sabha colors remain unchanged. Also updated project memory to reflect the new durable palette rule. Verification: `PATH=/opt/homebrew/bin:$PATH npm run build --prefix frontend`.
- Files touched: `api_router.py`, `frontend/lib/account.js`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Users may need to sign out and back in before all non-sidebar surfaces pick up the refreshed aspirant `theme_color` from their stored session.

- Date: 2026-06-08
- Request: Push the aspirant gold palette update to GitHub for deployment.
- Summary: Pushed commit `3880fe47` (`Set aspirant palette to gold`) to `origin/main`, publishing the aspirant color update so the backend auth/session theme payload and the shared sidebar theme helper now use `#DAB433` for aspirant MP/MLA accounts.
- Files touched: `TASK_LOG.md`
- Risks or follow-ups: Unrelated local edits in `data/geography/Ghaziabad/Loni.json` and `tenant_overrides.json` remained intentionally unpushed. Users may need to sign out and back in before all stored-session surfaces reflect the refreshed aspirant theme color.
...
- Date: 2026-06-09
- Request: Create a repeatable production seed script for tenant 10 contact-thread demo cases and run it on the live database.
- Summary: Added `scripts/seed_contact_thread_demo.py`, an idempotent operational seed script that upserts a demo tenant/profile/user and inserts three curated Briefcase cases showing `valid_multi_issue`, `high_frequency`, and `spam_suspected` contact-thread states. Because live Postgres currently stores `cases.assigned_to` as an integer while the ORM model still declares it as `String`, the script inserts case rows with explicit SQL instead of ORM `Case` objects. Copied the script to EC2, ran it inside the production backend container for tenant `10`, and verified the live rows `NDL-T10-001`, `NDL-T10-002`, and `NDL-T10-003` with distinct counts `3`, `6`, and `10`. Also temporarily opened SSH only for the seed run and revoked that rule immediately afterward.
- Files touched: `scripts/seed_contact_thread_demo.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: The script is safe for repeat use on the curated demo rows it owns, but the underlying `cases.assigned_to` ORM/schema drift should be fixed centrally before any broader operational scripting relies on `Case` inserts.
- Date: 2026-06-09
- Request: Redesign 24-hour citizen threads so each complaint is its own real case with its own geography and resolve flow.
- Summary: Replaced the old metadata-only contact-thread model for new intake with a sibling-case architecture. Distinct complaints from the same citizen inside the 24-hour window now create separate `cases` rows tied by `case_metadata.contact_thread_id`, while duplicate/low-information follow-ups still merge into the matching case’s `contact_message_events`. The Briefcase `/api/cases` response now groups sibling cases into one visible thread row with `thread_case_ids`, `thread_case_count`, and updated `pending_contact_count` / `distinct_issue_count`, and `/api/cases/{id}` now exposes `thread_cases` so the drawer can show each complaint as an independently actionable case with its own location, status, assignment, and quick resolve button. Kept a compatibility fallback for older `contact_thread_items` rows, then updated targeted and full intake/Briefcase regressions plus the frontend production build.
- Files touched: `main.py`, `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/hooks/useBriefcaseCases.js`, `tests/test_e2e_core_flow.py`, `tests/test_briefcase_api.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: The new grouping happens in Python at the API layer rather than through dedicated DB columns, which is acceptable for pilot scale but may need a first-class indexed thread column if case volumes grow. Older live threads created under the previous metadata-child model remain readable, but only new intake uses the sibling-case structure.

- Date: 2026-06-10
- Request: Deploy the Briefcase reliability fixes to production.
- Summary: Fast-forwarded `main` to commit `ab599d6d` (`Briefcase reliability: AI-hint-only intake, grounding gate, venue suppression, geo level lock, seat scoping, unknown-over-guess`) in a clean temporary worktree so the local uncommitted edits in `main.py`, `tenant_overrides.json`, and `data/geography/Ghaziabad/Loni.json` stayed untouched. This push publishes the AI-hint-only category/location handling, grounding gate, venue-word suppression, parent/child geography abstention, strict seat scoping, unknown-over-guess behavior, deterministic civic rescue rules, prompt tightening, and the new Briefcase guardrail test coverage. This `main` push is intended to trigger the EC2 backend deploy workflow.
- Files touched: `TASK_LOG.md`
- Risks or follow-ups: The push path intentionally avoided the dirty local working tree, but production verification should still confirm the backend rollout completed and that ambiguous category/location cases now land in review instead of being guessed.

- Date: 2026-06-11
- Request: Implement the first high-value admin operations fixes without a broad redesign.
- Summary: Made tenant detail the clearer canonical admin operations surface by adding anchored tenant operations for profile, WhatsApp, staff, geography corrections, and launch readiness. Updated setup checklist and low-profile alert routing to send operators back through tenant detail context, and demoted duplicate registry editors by replacing the old WhatsApp config form and legacy geography bulk editor with links to the tenant detail page and tenant-aware Shared Geography workspace. Added focused regression coverage for setup routing and registry ownership boundaries. Verified with `PATH=/opt/homebrew/bin:$PATH npm run test --prefix admin -- --run tests/setup-checklist.test.jsx tests/profile-editor.test.jsx tests/geography.test.jsx` and `PATH=/opt/homebrew/bin:$PATH npm run build --prefix admin`.
- Files touched: `admin/app/dashboard/mps/[tenant_id]/page.js`, `admin/app/dashboard/mps/[tenant_id]/setup/page.js`, `admin/app/dashboard/page.js`, `admin/components/admin-domains/accounts/ProfileEditorPage.jsx`, `admin/tests/setup-checklist.test.jsx`, `admin/tests/profile-editor.test.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: This is intentionally a first ownership slice, not the full admin redesign. Profile/credential editing still lives in Accounts registry for now, while WhatsApp/staff/geography operations route through tenant context. Persistent jobs/alerts and scalable server-side tenant search remain future admin hardening work.

- Date: 2026-06-11
- Request: Push the first admin tenant-operations ownership slice.
- Summary: Pushed commit `3d34953a` (`Clarify admin tenant operations ownership`) to `origin/main`, publishing the tenant detail operations anchors, setup/alert routing through tenant context, registry demotion for duplicate WhatsApp/geography editors, and focused admin regression coverage. This `main` push is intended to trigger the connected admin Vercel deployment.
- Files touched: `TASK_LOG.md`
- Risks or follow-ups: The push was prepared from a clean `main` worktree so unrelated local edits in the primary workspace stayed unpushed. Persistent jobs/alerts, server-side tenant search, and a broader geography split remain future admin hardening work.
- Date: 2026-06-22
- Request: Build the full inbound WhatsApp durability slice so every inbound webhook is ledgered before `200`, stuck rows are swept, and admin operators can inspect/retry failures.
- Summary: Finished the durable inbound WhatsApp ledger rollout. `main.py` now persists inbound payloads into `wa_inbound_messages`, dispatches processing from that ledger, and runs startup/background rescue sweeps for rows stuck in `received` or stale `processing`. `sansadx_backend/db.py` now models the ledger fully, including `last_received_at`. `admin_api.py` now exposes inbound queue listing/retry APIs and shifts system health + alerts to real inbound transport status instead of guessing from `cases`. Added a focused System route at `/dashboard/system/whatsapp-inbound` for operators to inspect failed/stale rows and trigger retries. Verified with `venv/bin/python -m pytest -q tests/test_whatsapp_inbound_ledger.py` and `PATH=/opt/homebrew/bin:$PATH npm run build:admin`.
...
- Files touched: `TASK_LOG.md`
- Risks or follow-ups: The clean-main worktree used for push does not carry the local virtualenv or installed frontend dependencies, so verification was performed in the primary workspace before the cherry-pick. Post-deploy smoke checks should confirm `/api/admin/system-health`, `/api/admin/whatsapp/inbound`, and `/dashboard/system/whatsapp-inbound` are live and that new inbound webhook rows move from `received` to `processed`.

- Date: 2026-07-11
- Request: Decide and implement how multiple messages from one number should behave (same-case follow-ups vs different-case complaints), after verifying the merge/distinct mechanism works.
- Summary: Verified the thread mechanism end-to-end (near-duplicates merge via the tiered similarity gate, distinct complaints become sibling cases in one thread) and then shipped the new bounded ack contract in two phases. Phase 0 restored the "ack first, clarify later" policy for text complaints (immediate details-request is now media-lane-only) and fixed the stale notify test stub. Phase 1 added the ack policy: full ack for the 1st issue, short localized separate-issue acks for the 2nd-5th, one reassurance per thread for duplicates/nudges, a one-time boundary notice at the 6th issue, silence beyond; no case reference numbers anywhere by explicit product decision. Templates in Hindi/Hinglish/Marathi/Kannada/English. Verified with the full e2e intake suite (11 tests incl. new nudge-reassurance test) plus clarification/media/localized/text-buffer/ledger suites; deployed both phases to EC2 (commits e553d21f, d565f9ef) with healthy post-deploy checks.
- Files touched: `main.py`, `modules/localized_replies.py`, `tests/test_e2e_core_flow.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Phase 2 (burst digest ack collapsing several near-simultaneous distinct-issue acks into one message) is deliberately deferred. The similarity gate is lexical, so a same-issue follow-up in a different language becomes an extra sibling case with an extra short ack — conservative but worth a semantic pass later. A real handset smoke test of the new replies on tenant 10 is still pending.

- Date: 2026-07-12
- Request: Design and implement a system guaranteeing consistent citizen acknowledgements regardless of the underlying LLM (fixed communication policy, template/AI split, anti-creativity controls, no false promises, no politics/opinions, validation layer, architecture changes).
- Summary: Grounded review found three lanes: intake acks were already deterministic templates; the classifier's free-prose political_response and the Briefcase notify lane were the real drift sources, and the notify fallback dict itself violated policy (case refs, "Good news!", a reply-'NO' reopen mechanism that does not exist). Shipped the policy layer: modules/ack_policy.py (versioned constitution + violation patterns), modules/ack_composer.py (fixed status templates x Hindi/Hinglish/Marathi/Kannada/English with a pre-translated taxonomy issue-phrase dictionary; no model output in any citizen sentence), modules/ack_validator.py (deterministic checks; ACK_POLICY_MODE shadow default / enforce), and wired both notify endpoints to validate before persisting/sending, replacing the fallback dict with localized composer output. Golden tests pin byte-identical strings; suites prove every composer output and every existing intake template passes the validator. Repaired three pre-existing stale test stubs in test_briefcase_api.py. Deployed to EC2 (commit faabbaa) with healthy post-deploy check; in-container smoke was skipped because outbound SSH is blocked from the current network — behavior is pinned by the golden tests that ran against the deployed commit.
- Files touched: `modules/ack_policy.py`, `modules/ack_composer.py`, `modules/ack_validator.py`, `api_router.py`, `tests/test_ack_policy_system.py`, `tests/test_citizen_notify.py`, `tests/test_briefcase_api.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Run shadow mode for ~a week and review ack_policy_flag case activity before setting ACK_POLICY_MODE=enforce. Remaining follow-ups: retire classifier political_response as citizen-facing text, surface enforce rejections inline in the Briefcase notify UI, and extend the issue-phrase dictionary beyond the starter five subdomains. Two stale port-22 security-group rules ("SSH from current IP" 106.219.126.80, "Codex temporary SSH" 49.36.179.242) predate this session and should be reviewed/removed.

- Date: 2026-07-13
- Request: Fix the reported bug where a citizen's non-complaint message ("But we need action. Dont just register complaints") was misclassified as a new complaint, spawning a phantom 3rd sibling case, a "separate issue" ack, and a location-clarification demand. Design and build Phase A (tenant-scoped heuristic detection) of a two-layer CASE_COMMENT system, per user-directed refinements: broader category (not just frustration — gratitude/urging/status-inquiry/pending), name CASE_COMMENT, tenant-specific Layer 1 language dictionaries with AI as multilingual fallback (Phase B, deferred), log-only staff visibility (no Briefcase badge yet).
- Summary: Built modules/tenant_languages.py (tenant communication-language resolution: config override > state default table > Hindi/Hinglish/English fallback, in-process cached) and modules/case_comment_heuristics.py (tenant-language-scoped phrase buckets with a 12-word cap to avoid hijacking real longer grievances). Consolidated main.py's two duplicated low-information-nudge blocks into one _handle_low_info_or_case_comment helper that also runs the new heuristic; a match logs a case_comment event (reusing the existing _append_contact_message_event mechanism) and replies via the existing reassurance lane — no case, no distinct-issue bump, no location follow-up. Along the way found and fixed a latent bug in _update_contact_thread_meta where logging any event on a non-anchor sibling case silently reset its distinct_issue_count to 1. New e2e test reproduces the exact reported scenario end-to-end and asserts no phantom case is created; 86 tests pass across all affected suites (case_comment_heuristics, tenant_languages, ack_policy_system, citizen_notify, localized_replies, e2e_core_flow, briefcase_api, citizen_clarification_flow, whatsapp_text_buffer). Deployed to EC2 (commit 99800f7), backend health confirmed; in-container live smoke skipped since outbound SSH from this environment timed out (network-level, not an auth issue) — behavior is fully pinned by the test suite that ran against this exact commit.
- Files touched: `modules/tenant_languages.py`, `modules/case_comment_heuristics.py`, `main.py`, `tests/test_case_comment_heuristics.py`, `tests/test_tenant_languages.py`, `tests/test_e2e_core_flow.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Phase B (AI CASE_COMMENT classifier status in sansadx_backend/prompts.py, as the fully multilingual fallback for phrasing Layer 1's tenant heuristics miss) and Phase C (media-with-minimal-caption-in-active-thread attaches as evidence to the most recent open case instead of spawning a new one) are both deferred, user-approved next phases, not yet built. Layer-1 phrase coverage currently spans English/Hindi/Hinglish/Marathi/Kannada only — other tenant languages fall through to old behavior until Phase B ships. Revisit the Briefcase badge decision once case_comment event volume/tone data exists.

- Date: 2026-07-13
- Request: Phase B of the CASE_COMMENT system — AI classifier status as the fully multilingual fallback for phrasing Phase A's tenant-scoped heuristics don't cover.
- Summary: Added CASE_COMMENT to the classifier's status enum in sansadx_backend/prompts.py (with a comment_tone field and an explicit distinguishing rule so a message naming a real new problem is still classified normally even if it also sounds urgent), plus three few-shot examples. While drafting the examples, caught and fixed a self-contradiction: an early draft used "Emergency & Disaster Response" as a domain, which doesn't exist in unified_taxonomy.py's 9 canonical categories — replaced with a verified real domain/subdomain/program pairing before it could ship a bad few-shot example that violated the prompt's own "never invent taxonomy labels" rule. Added "case_comment" to ai_engine.py's _VALID_STATUSES whitelist (without this the new status would have been silently coerced to "pending", making the whole prompt change inert) and grouped CASE_COMMENT with OFFENSIVE/IRRELEVANT for taxonomy null-out and emergency-override exclusion. Added main.py's _handle_ai_case_comment helper, wired into both thread paths' distinct-issue branches right after AI enrichment, before a sibling case would be created — same no-case/no-bump/no-location-followup/one-reassurance treatment as Phase A. New tests at both layers: ai_engine-level tests (CASE_COMMENT and comment_tone survive normalization, unknown status still falls back to pending, mismatched tone gets nulled) and an e2e test driving a phrase Phase A's heuristics do not catch through the real intake path, confirming no phantom case. 67 tests pass across all affected suites; 3 pre-existing unrelated failures in test_ai_location_grounding.py (documented earlier this session, predate this work) are unaffected. Deployed to EC2 (commit 0a7c753), backend health confirmed.
- Files touched: `sansadx_backend/prompts.py`, `sansadx_backend/ai_engine.py`, `main.py`, `tests/test_ai_location_grounding.py`, `tests/test_e2e_core_flow.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Phase C (media with minimal caption in an active thread attaches as evidence to the most recent open case rather than spawning a new one) is the last deferred phase, not yet built. The rare edge case of CASE_COMMENT as a citizen's very first-ever message (no active thread, nothing to attach a comment to) is intentionally left unhandled — the raw case row from STEP 1 just keeps status="case_comment" and stays visible in Briefcase's default queue (confirmed not excluded from any status filter), a cosmetic rough edge rather than a data-loss risk. Live in-container smoke test still not possible from this environment (SSH times out at the network level); behavior is fully pinned by the test suite that ran against this exact deployed commit.

- Date: 2026-07-13
- Request: Phase C of the CASE_COMMENT system — media (photo/PDF) arriving with no caption, or a low-information caption, inside an active thread should attach as evidence to the most recent open case instead of spawning a new sibling case.
- Summary: Added `_handle_contextless_media_attachment` in `main.py`, wired into both thread paths right before the existing CASE_COMMENT checks (media doesn't need the classifier's judgment call, so it's a plain deterministic rule, not folded into CASE_COMMENT). It reuses `_is_low_information_followup` on the caption (empty or a nudge like "sir"/"update" both count) and, on a match, saves the file via a new `_attach_media_to_case` (an existing-case sibling of `_create_raw_contact_case`'s media insert, with the same `meta_message_id` dedup guard), logs a `media_evidence_attached` thread event, and answers through the existing reassurance lane — no case, no distinct-issue bump, no location follow-up. Deliberately restricted to `media_type in {"image", "document"}`: WhatsApp's API has no caption field for audio (`_dispatch_inbound_ledger_row` always passes `caption=""` for voice notes), so an empty string there is the absence of the field, not a real "no caption" signal — treating it as one would have silently attached every follow-up voice note as evidence and made it impossible to file a new complaint by voice. Four new e2e tests cover: contextless photo attaches to the existing case with no phantom case; a low-info caption ("update") still attaches; a caption naming a real new problem still creates a genuine second case; and audio with an empty caption is NOT swallowed (creates its own case). While writing the tests, found and fixed a real test-isolation gap in the shared `_seed_database()` harness: it deleted `cases` between tests but never `case_media`, so a leftover media row from an earlier test collided with a reused SQLite row id in a later test — added `case_media` to the per-test cleanup list. Full `tests/test_e2e_core_flow.py` (21 tests) and the CASE_COMMENT/media-focused subset pass; confirmed via `git stash` that the ~37 failures seen when running the entire `tests/` directory in one process are pre-existing cross-file state pollution unrelated to this change (present identically before and after).
- Files touched: `main.py`, `tests/test_e2e_core_flow.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Deployed to EC2 via the `Deploy Backend To AWS EC2` GitHub Actions workflow (commit `42dbf34`, run `29234047136`), which passed its own `Verify Backend Health` step; `https://api.theneedle.in/health` independently confirmed `200` after rollout. Direct SSH from this environment to the EC2 host timed out at the network level again (same pre-existing limitation noted in the Phase A/B entries) so no in-container smoke test was run; behavior is pinned by the test suite that ran against this exact deployed commit.
- Risks or follow-ups: All three phases (A: tenant-scoped heuristics, B: AI multilingual fallback, C: contextless media attachment) of the CASE_COMMENT system are now built and deployed; recommend a short observation window on `case_comment`/`media_evidence_attached` event volume before deciding on the Briefcase badge (deliberately deferred in the original design discussion). The pre-existing full-`tests/`-directory cross-file pollution (37 unrelated failures) is worth a separate cleanup pass — likely needs either per-file pytest invocation in CI or fixing shared global state (`main.engine`, `dbmod.SessionLocal`) across test files that don't expect to share a process.

- Date: 2026-07-24
- Request: Stop patching geography string-matching and redesign geography intelligence so wrong mappings become increasingly rare over time (user-directed refinements: spatial relations like "behind bus stand" parsed as relation+anchor; discovery of new places is the system's job, not staff's). Also fix the CI rate-limiter cascade that kept Product Tests red since 2026-07-09.
- Summary: Shipped Phase 1 of the gazetteer redesign in shadow mode. New closed place-entity registry (geo_places/geo_place_variants with per-variant provenance and candidate/verified/deprecated status, authored parent edges — hierarchy is never inferred from strings), relation lexicon parser (Hindi/Hinglish/Marathi/Kannada/English postpositions and prepositions: "X ke peeche wale compound mein" → anchor X + relation behind, descriptive tail discarded), and resolver v2 with seat-scoped candidate lookup (generic words structurally cannot match — no blocklists exist), length-scaled edit budgets (≤4 chars exact-only), citizen-history scoring boost, and a three-way accept/ask/abstain decision that never coin-flips across assemblies. Learning loop: save_overrides_to_db now harvests every manual correction into a learned variant; unresolved spans cluster into a discovery queue ranked by distinct citizens with one-tap promote (admin endpoints: gazetteer/import, discovery-queue, discovery-queue/{id}/promote, shadow-stats). v2 is wired after _finalize_whatsapp_geography_decision in shadow only — logs full evidence traces + legacy agreement to geo_resolution_log, never changes citizen-facing outcomes. 24 new tests including golden regression cases encoding the historical failure classes (generic-word phantom matches, short-name fuzzy coin flips, same-name-two-assemblies guesses, relation-fragment pollution). CI fix: autouse limiter.reset() in conftest.py (SlowAPI in-memory counters accumulated across the pytest process; ~33 tests failed 429 on every push since Jul 9). That fix exposed two stale harnesses, both repaired: the lockout test needed a mid-test limiter reset (SlowAPI's 429 fired before the app's 423 lockout), and test_pilot_readiness patched JWT_SECRET but not the ADMIN_JWT_SECRET constant split off on 2026-07-08.
- Files touched: `modules/gazetteer.py`, `modules/geo_relations.py`, `modules/geo_resolver_v2.py`, `sansadx_backend/db.py`, `main.py`, `admin_api.py`, `tests/conftest.py`, `tests/test_geo_relations.py`, `tests/test_geo_gazetteer_v2.py`, `tests/test_auth_password_flows.py`, `tests/test_pilot_readiness.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: v2 is shadow-only — run POST /admin/geo/gazetteer/import once after deploy to seed entities, let shadow-stats accumulate ~2 weeks, then decide the champion flip on agreement + golden-set data. Phase 2 (deferred): auto-promotion guardrails going zero-touch, relation-as-scoring-signal, "ask" branch wired to the citizen clarification flow, deleting the legacy resolver + main.py's inline fallback scan after the flip. Discovery-queue UI in admin frontend not yet built (API-only). Note: tests/test_geography_onboarding_api.py dirties data/geography/Ghaziabad/Loni.json via the dev JSON fallback save path (pre-existing pollution, also visible in the Desktop checkout) — restore before committing, worth a proper fix later.

- Date: 2026-08-24
- Request: Fix government complaint status checks so the view shows where the complaint is currently sitting even when the broad status has not changed; include Karnataka as well as Rajasthan.
- Summary: Added `StatusResult.portal_detail` and wired Rajasthan's existing PII-trimmed detail plus Karnataka iPGRS's department/category/pendency/date fields into it. Successful manual, interactive, and background portal checks now write a `status_polled` audit payload even when `cases.govt_status` is unchanged, with `changed=false`, `raw_portal_status`, and `portal_detail`. `/api/cases/{id}/govt` returns the latest check as `latest_status_check`, and the Briefcase government filing section renders it as `Current portal position` with a clearer unchanged-status toast.
- Files touched: `api_router.py`, `modules/govt_sync/adapters/base.py`, `modules/govt_sync/adapters/karnataka_ipgrs.py`, `modules/govt_sync/poller.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `tests/test_karnataka_status_check.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_karnataka_status_check.py -q`; `venv/bin/python -m pytest tests/test_maharashtra_status_check.py tests/test_govt_duplicate_filing.py -q`; `venv/bin/python -m py_compile api_router.py modules/govt_sync/adapters/base.py modules/govt_sync/adapters/karnataka_ipgrs.py modules/govt_sync/poller.py`; `npm run build --prefix frontend`.
- Risks or follow-ups: The display uses the latest stored successful check; existing old cases will show the richer current position only after the next successful status check. The code intentionally still avoids Rajasthan's unauthenticated history endpoint and stores only trimmed detail from the already authorized status-detail call.

- Date: 2026-08-24
- Request: Commit, push, and deploy the government portal status-position fix.
- Summary: Committed the fix on the working branch as `99e20598` (`Show latest govt portal position`), pushed branch `claude/govt-sync-maharashtra-status-check`, cherry-picked it onto a clean `origin/main` worktree as `c44247a5`, and pushed `main` to trigger production deployment. The `Deploy Backend To AWS EC2` workflow run `32715888152` completed successfully and its backend health check passed; `https://api.theneedle.in/health` returned `{"status":"ok","system":"Needle Backend V8.2"}` after deployment.
- Files touched: `TASK_LOG.md`
- Verification: Clean-worktree `venv/bin/python -m pytest tests/test_karnataka_status_check.py -q` passed; clean-worktree backend `py_compile` passed; the earlier primary-workspace `npm run build --prefix frontend` passed. GitHub Product Tests run `32715888301` failed later in MP Playwright e2e selectors (`Review Now`, `Bureaucratic / Administrative`, and `Infrastructure & Utilities` expectations), after backend/admin/unit suites passed; those failures are outside the govt status-position path and should be handled separately.
- Risks or follow-ups: The deployed backend code is live on EC2. MP Playwright e2e expectations need a separate freshness pass so CI is green again.

- Date: 2026-08-24
- Request: Build Maharashtra Aaple Sarkar interactive government status-check integration, following a live-protocol trace supplied by the user (verify-identity+CAPTCHA -> OTP+CAPTCHA -> registration/token+CAPTCHA+cid -> final CAPTCHA+cid submission), reusing the InteractiveStatusCheckMixin architecture introduced for Karnataka. Then, after review, investigate and fix a status-normalization gap before approval.
- Summary: Added `modules/govt_sync/adapters/maharashtra_aaplesarkar.py` (`MaharashtraAapleSarkarAdapter(InteractiveStatusCheckMixin, ManualAssistedAdapter)`), a 3-stage `HTML_FORM_POST` flow with CSRF-token/CAPTCHA rotation per stage, an in-process TTL-based attempt store (mirrors `browser_session.py`'s `_sessions` precedent, not a DB table), and a result-page parser (`_extract_label_value`) built and fixed against a real captured result-page structure. Registered the adapter in `adapters/__init__.py` and `modules/data/govt_portals.json` (`status_check_adapter: "maharashtra_aaplesarkar_api"`; portal is currently INERT in production because EC2 cannot reach `grievances.maharashtra.gov.in` on 80/443 — DNS resolves, TCP times out, reconfirmed live via the `diagnose-portal-reachability` workflow). Generalized the interactive-status API/UI contract from Karnataka's single-CAPTCHA shape to a multi-stage one: `GovtStatusCheckAdvanceRequest` gained an optional `otp` field, the intermediate `awaiting_human_input` response now carries `pending_human_verification` (previously only `/start` did), and `BriefcaseCaseModal.jsx` renders CAPTCHA and/or OTP per stage from a `pending` array — additive only, Karnataka's own request/response shape and tests are unaffected. Own testing (not assumption) surfaced two real gaps before shipping: a newline-handling bug in `_extract_label_value` (label/value land on separate lines after `get_text("\n", strip=True)`; `.lstrip()` doesn't strip `\n`) fixed directly, and a status-taxonomy gap where `normalize_status_keywords("Submitted")` returned `None` because the `submitted` bucket's keyword list never included the literal word "submitted". The taxonomy gap was investigated and reported (not fixed) pending explicit approval; once approved, applied the exact one-line fix to `STATUS_KEYWORDS` in `modules/govt_sync/adapters/base.py` (added `"submitted"` to the existing bucket) and re-validated: Maharashtra suite 19/19, Karnataka suite 16/16, govt duplicate-filing suite 12/12, full backend suite (14 pre-existing/unrelated failures, confirmed via isolation runs — cross-file pollution + one already-documented flaky lockout-timing test), py_compile clean, a fresh local-Postgres Rajasthan validation run, and explicit `normalize_status_keywords()` checks confirming Maharashtra "Submitted" -> "submitted", Karnataka "Registered & Sent for Scrutiny" -> "submitted" (unaffected, pre-existing), and all `resolved`/`rejected`/`under_review` examples unchanged.
- Files touched: `modules/govt_sync/adapters/maharashtra_aaplesarkar.py`, `modules/govt_sync/adapters/__init__.py`, `modules/govt_sync/adapters/base.py`, `modules/data/govt_portals.json`, `tests/test_maharashtra_status_check.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_maharashtra_status_check.py -q` (19/19); `venv/bin/python -m pytest tests/test_karnataka_status_check.py -q` (16/16); `venv/bin/python -m pytest tests/test_govt_duplicate_filing.py -q` (12/12); full `pytest tests/` (14 pre-existing/unrelated failures, isolated and confirmed); `py_compile` across all touched modules; fresh local-Postgres Rajasthan OTP-flow validation script; direct `normalize_status_keywords()` checks for Maharashtra/Karnataka/resolved/rejected/under_review examples.
- Risks or follow-ups: The Maharashtra adapter is code-complete and fully mock-tested but cannot be live-tested end-to-end until the EC2-to-portal network block is resolved (separate infra task, not a code defect). CAPTCHA/OTP are never solved or bypassed by the adapter — a human staff member supplies both at each stage via the Briefcase UI.

- Date: 2026-08-24
- Request: Commit, push, and deploy the approved Maharashtra status-normalization fix and the Maharashtra Aaple Sarkar adapter.
- Summary: Committed the base.py one-line taxonomy fix plus the full Maharashtra adapter build (`db3c08ee`) on `claude/govt-sync-maharashtra-status-check`, rebased onto `origin/main` (resolving a straightforward TASK_LOG.md append conflict against the user's own concurrent "Show latest govt portal position" / deploy-log commits, keeping both entries), re-ran the full Maharashtra/Karnataka/govt-filing suite (47/47) and py_compile clean on the rebased tree, then pushed to `main` (`git push origin HEAD:main`, `60b214a9..db3c08ee`). The `Deploy Backend To AWS EC2` GitHub Actions workflow (run `32720471238`) completed successfully with its `Verify Backend Health` step passing; `https://api.theneedle.in/health` independently confirmed `200 {"status":"ok"}` after deployment.
- Files touched: `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_maharashtra_status_check.py tests/test_karnataka_status_check.py tests/test_govt_duplicate_filing.py -q` (47/47) on the rebased tree; `py_compile` on `base.py`/`maharashtra_aaplesarkar.py`/`adapters/__init__.py`/`api_router.py`; `gh run watch 32720471238` completed success; live `curl https://api.theneedle.in/health` confirmed `200` post-deploy.
- Risks or follow-ups: Maharashtra adapter remains INERT in production pending resolution of the EC2-to-portal network block (separate infra task). Product Tests run for this push was still in progress at commit time — not yet confirmed green.

- Date: 2026-08-25
- Request: Diagnose and resolve the Maharashtra EC2-connectivity block (previously left as a documented open blocker), then run the first real human-assisted end-to-end Maharashtra status-check test and extend the parser based on real evidence.
- Summary: Investigation (read-only AWS inspection + a controlled temporary secondary-EIP experiment, fully rolled back) proved the block was specific to the backend's original Elastic IP `3.6.228.105`, not an AWS-wide restriction — the identical EC2 host reached Google and Rajasthan Sampark instantly throughout. Migrated production to a new permanent EIP, `52.66.71.161`, via a checkpointed cutover (AWS EIP reassociation -> SSH verification -> `AWS_EC2_HOST` GitHub secret update -> existing ops-workflow verification -> GoDaddy DNS update for `api.theneedle.in` -> full health/webhook/Maharashtra-reachability re-verification), keeping the old EIP allocated (not released) for rollback. No application code or workflow files were touched for the migration itself. With connectivity restored, ran a real human-assisted 3-stage Maharashtra check (CAPTCHA -> OTP -> CAPTCHA) against a genuine pre-existing citizen grievance (case #2446, tenant 3, reference `DEP/RDDE/BULD/2026/183`) after setting the tenant's previously-blank `govt_contact_primary_number`; the real portal returned "Submitted", correctly normalized to `submitted`, `checked=true` — confirming the earlier `base.py` fix works against live data, not just mocks. The user then captured the actual live result-page HTML (via browser dev tools), which showed a `#customers` table with Department/Office/Officer/Office Contact/Office Email fields the adapter had never extracted (an intentional, narrower original scope, documented at build time). Validated a targeted extraction approach directly against that real captured HTML before touching any file, then applied the approved diff: a new `_extract_table_field()` helper plus extending `_parse_result_html()`/`_advance_stage2()` to populate `StatusResult.portal_detail` with those fields (uploaded-document/complaint-image cells deliberately excluded, staying PII-trimmed like Karnataka's own `portal_detail`). Replaced the test suite's old constructed-approximation HTML fixture with the real captured page.
- Files touched: `modules/govt_sync/adapters/maharashtra_aaplesarkar.py`, `tests/test_maharashtra_status_check.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_maharashtra_status_check.py -q` (19/19); `venv/bin/python -m pytest tests/test_karnataka_status_check.py -q` (16/16, regression); `venv/bin/python -m pytest tests/test_govt_duplicate_filing.py -q` (12/12, regression); `py_compile` clean; one real live 3-stage portal check completed successfully end-to-end (production evidence, not mocked); extraction logic validated directly against the real captured HTML before the diff was written.
- Risks or follow-ups: Old EIP `3.6.228.105` intentionally kept allocated for a >=7 day rollback window before any decision to release it. A second live round through the same real portal failed at the final CAPTCHA (a misread, not a code defect) — harmless, session was already single-use and expired cleanly. Whether the new EIP itself could eventually be flagged the same way is unverified and worth a longer observation window.

- Date: 2026-08-25
- Request: After deploying the Maharashtra portal_detail parser extension, the live "Current portal position" panel on case #2446 still only showed "Portal status: Submitted" — none of the new district/department/office/officer/contact/email fields appeared. Diagnose and fix.
- Summary: Confirmed the backend was correct (real deployed code, tests) — the gap was in `frontend/components/briefcase/BriefcaseCaseModal.jsx`'s `formatGovtPortalDetailRows()`, whose row list was hardcoded to Karnataka's `portal_detail` key names (`department_name`, `category`, `pendency_details`, `subject`, `grievance_date`, ...) and silently dropped any unrecognised key — so Maharashtra's new fields (`district`, `department`, `office`, `officer`, `office_contact`, `office_email`) never had anywhere to render, even though the API response already carried them correctly. Added rows for District/Office/Officer/Office contact/Office email, and made the existing Department row fall back to Maharashtra's `department` key when Karnataka's `department_name` isn't present — purely additive, Karnataka's own rows/labels unchanged.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `TASK_LOG.md`
- Verification: `npm run build --prefix frontend` (clean, all 18 routes built successfully).
- Risks or follow-ups: Frontend deploys via Vercel's own GitHub integration (no workflow file/CLI access in this environment to directly trigger or watch it) — confirm the live site reflects this after Vercel's automatic build completes.

- Date: 2026-08-26
- Request: Build a local-only experimental Government Filing Workspace prototype for Karnataka, reusing the existing live browser-session architecture; do not deploy, do not touch real CAPTCHA/OTP/submission, and do not implement guessed portal-field autofill.
- Summary: Added a `NEXT_PUBLIC_GOVT_FILING_WORKSPACE_SPIKE=true` frontend gate that changes only the live-session presentation in Briefcase into an experimental Government Filing Workspace. The workspace reuses the existing `GovtLiveBrowserView` canvas/WebSocket stream, shows the real streamed browser view on the left, shows Needle-derived `Prepared Fields / Copy Source` values on the right, and displays visible interaction counters/last-action text for clicks, keystrokes, scrolls, and mouse movement so the spike can demonstrate interactivity rather than a passive screenshot/video. No portal-field autofill was added; the final-submit warning explicitly states Needle does not click Submit. Also corrected a stale backend comment that still claimed live sessions autofilled fields.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `api_router.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_govt_duplicate_filing.py -q` (12/12); `venv/bin/python -m py_compile api_router.py`; `npm run build --prefix frontend` (clean, all 18 routes built successfully). Local screenshot run used tenant 1 / case 12 from `test_govt_duplicate_filing.db`, with the Karnataka portal URL pointed only at a localhost mock portal (`http://127.0.0.1:8765`) and the MP frontend started with `NEXT_PUBLIC_GOVT_FILING_WORKSPACE_SPIKE=true`; screenshots saved under `/private/tmp/needle-filing-*.png`. Python 3.14 venv could not run Python Playwright locally, so a temporary Python 3.11 venv under `/private/tmp/needle-govt-spike-py311` was used for the local backend screenshot test.
- Risks or follow-ups: This is deliberately a LOCAL/MOCK spike, not production behavior. Production was not deployed or modified. No real government portal, CAPTCHA, OTP, or grievance submission was used. Before productionizing, require a separate review for terms/permissions, portal reachability from the filing host, worker affinity, session limits, audit logging, and an explicit decision on whether any verified selector-based prefill is acceptable.

- Date: 2026-08-27
- Request: Make the Briefcase case modal visually match `Main-2.pdf` while preserving all existing actions.
- Summary: Updated the modal's first-screen presentation to match the PDF structure: the drawer now leaves the left dashboard sidebar visible, uses a compact PDF-style header/meta row, shows the complaint strip as small dated cards, moves the large case title/chips into the top content area, and adds a horizontal government grievance journey card above the detailed complaint sections. Preserved and re-wired the action controls for Follow/Unfollow, More → copy case reference, Priority updates, status changes, assignment, geography save, and government filing.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
...
- Date: 2026-09-03
- Request: Implement Phase 1-8 of the generic Government Status Snapshot layer without changing existing state adapter behavior or adding UI/history APIs.
- Summary: Added an additive snapshot observer service plus ORM/startup DDL for `govt_status_snapshots`, `govt_status_snapshot_fields`, and `govt_status_snapshot_events`. Wired best-effort snapshot persistence only after existing successful `status_polled` paths in manual API polling, interactive status completion, Tamil Nadu live status completion, and the background poller. Kept existing current-state fields and `govt_submission_log` behavior as the compatibility contract; no frontend history UI/API was added.
- Files touched: `api_router.py`, `main.py`, `sansadx_backend/db.py`, `modules/govt_sync/poller.py`, `modules/govt_sync/status_snapshot.py`, `tests/test_govt_status_snapshot.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_tamil_nadu_status.py tests/test_karnataka_status_check.py tests/test_maharashtra_status_check.py tests/test_govt_otp_audit.py tests/test_govt_status_normalization.py tests/test_govt_status_manual_window.py tests/test_govt_portals_config.py tests/test_govt_sync_poller.py tests/test_govt_sync_poll_lock.py tests/test_govt_duplicate_filing.py tests/test_govt_status_snapshot.py -q` (130/130); `venv/bin/python -m py_compile modules/govt_sync/status_snapshot.py modules/govt_sync/poller.py api_router.py main.py sansadx_backend/db.py tests/test_govt_status_snapshot.py` (clean).
- Risks or follow-ups: No snapshot history frontend/API was implemented in this phase. Snapshot writes are deliberately non-deduplicated and best-effort; if future UI consumes these tables, design the read API separately with tenant scoping and event-noise rules.

- Date: 2026-09-03
- Request: Harden Phase 1-8 snapshot implementation only: align ORM/startup DDL semantics and add missing snapshot tests without starting Phase 9.
- Summary: Updated snapshot ORM metadata to match startup DDL cascade/index intent (`ondelete="CASCADE"` for case/snapshot child rows and explicit composite indexes), without adding dedupe or uniqueness constraints. Expanded snapshot tests from 4 to 18 cases covering partial snapshots, unavailable/redacted/explicitly-empty diff semantics, officer/status/multiple-field events, JSON key-order normalization, first-snapshot behavior, repeated identical read semantics, successful persistence compatibility, and non-fatal snapshot failure through the existing poll endpoint.
- Files touched: `sansadx_backend/db.py`, `tests/test_govt_status_snapshot.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_govt_status_snapshot.py -q` (18/18); requested govt regression bundle (144/144); full `venv/bin/python -m pytest tests -q` was practical and ran to completion with 599 passed / 23 failed in unrelated existing suites; `py_compile` clean on touched Python files.
- Risks or follow-ups: Full-suite failures were outside the snapshot/government regression bundle (missing `slowapi`, Briefcase/case-bucket 500s, seat/geography/webhook/pilot-readiness harness failures). No frontend/history API, adapter behavior, filing/auth/CAPTCHA/OTP behavior, or dedupe was added.

- Date: 2026-09-03
- Request: Implement Phase 9 GovtSync history API and Briefcase presentation from the approved contract.
- Summary: Added read-only `GET /api/cases/{case_id}/govt/history` with separate `current_state`, `latest_known`, and `latest_snapshot` concepts; public field states map internal `explicitly_empty` to `empty`; public events are limited to `status_changed`, `department_changed`, `officer_changed`, `comment_added`, and `reply_added`. Collapsed officer name/designation diffs into one `officer_changed` with `changed_parts`, added stable/derived/weak communication identity handling without DB uniqueness, and added a compact Briefcase history panel that keeps meaningful changes prominent, no-change checks quiet, and partial snapshots from blanking latest known values.
- Files touched: `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `modules/govt_sync/status_snapshot.py`, `tests/test_govt_status_snapshot.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_govt_status_snapshot.py -q` (26/26); requested government regression bundle (152/152); `venv/bin/python -m py_compile modules/govt_sync/status_snapshot.py api_router.py modules/govt_sync/poller.py main.py sansadx_backend/db.py tests/test_govt_status_snapshot.py` clean; `npm run build --prefix frontend` passed.
- Risks or follow-ups: Full unrelated suite was not rerun in this Phase 9 pass; previous hardening run showed unrelated failures outside GovtSync. No adapter behavior, filing/auth/CAPTCHA/OTP/session behavior, poller scheduling, submission behavior, raw capture storage, backfill, or dedupe constraints were added.

- Date: 2026-09-05
- Request: Migrate Karnataka's and Maharashtra's process-local, in-memory `_attempts` interactive status-check correlation state into PostgreSQL (approved Option B design), as a narrowly scoped infrastructure refactor — no portal/CAPTCHA/OTP/parsing/filing/Rajasthan/Tamil Nadu/Caddy/frontend changes.
- Summary: Added `GovtStatusCheckAttempt` ORM model + startup DDL for `govt_status_check_attempts` (attempt_id PK, tenant_id/case_id FKs, adapter_key, reference_number, mobile_or_email, cookies JSONB, csrf_token, stage, token, cid, created_at, last_activity_at; composite tenant/case index + last_activity_at index). Added `modules/govt_sync/status_attempts.py`, a small shared persistence helper exposing `create_attempt`/`load_attempt`/`update_attempt_stage`/`delete_attempt` — `load_attempt` is tenant/case/TTL-scoped in one WHERE clause (bound Python-computed cutoff timestamp, never string-interpolated `INTERVAL` SQL) so mismatched tenant, mismatched case, unknown attempt_id, and genuinely expired attempts are all indistinguishable (fail-closed); `update_attempt_stage`/`delete_attempt` are single atomic `UPDATE/DELETE ... WHERE ... RETURNING` statements (no read-then-write, no explicit row lock, no DB transaction spans a government-portal HTTP call) so two concurrent callers against the same attempt_id can never both succeed. Rewired `karnataka_ipgrs.py` and `maharashtra_aaplesarkar.py` to use this helper instead of their own module-level `_attempts` dict + `_gc_expired()`, removing the single-worker-affinity requirement `backend_govt_live`'s `WEB_CONCURRENCY=1` previously had to provide for these two adapters specifically (browser_session.py's live Playwright sessions still need that single-worker service for their own, separate reason, untouched here). Fixed a genuine concurrency bug found while writing the concurrency tests: Maharashtra's stage-failure handler was unconditionally calling `delete_attempt()` on any `_MaharashtraStageFailure`, including the "lost the stage-transition race" case — which would have deleted a concurrent winner's freshly-advanced row out from under it; added a `preserve_attempt` flag on that exception so only genuine terminal failures clear the row, while a lost race leaves the winner's state untouched. `api_router.py`: comment-only update at the `govt_status_check_advance` reconstruction site to describe the new store instead of the old process-local one.
- Files touched: `sansadx_backend/db.py`, `main.py`, `modules/govt_sync/status_attempts.py` (new), `modules/govt_sync/adapters/karnataka_ipgrs.py`, `modules/govt_sync/adapters/maharashtra_aaplesarkar.py`, `api_router.py`, `tests/test_govt_status_check_attempts.py` (new), `tests/test_karnataka_status_check.py`, `tests/test_maharashtra_status_check.py`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (198/198); `venv/bin/python -m py_compile` clean on all touched files; `git diff --stat` confirmed zero changes to `rajasthan_sampark.py`, `modules/govt_sync/status/tamil_nadu.py`, `browser_session.py`, Caddy/docker-compose config, and `frontend`/`admin`. Full unrelated suite (`pytest tests/ -q`) shows the same 23 pre-existing failures with this branch's changes present or fully stashed (auth/briefcase/case-buckets/dashboard-map/e2e/pilot-readiness/seat-registry/whatsapp-inbound-ledger) — confirmed pre-existing and unrelated to this migration, not introduced by it.
- Risks or follow-ups: No background sweep for abandoned rows was added (lazy expiration only, matching the approved design) — a future housekeeping pass may want one. No change to CAPTCHA/OTP/parsing/portal URLs/HTTP methods/payloads/status semantics, Rajasthan, Tamil Nadu, filing architecture, `backend_govt_live`, Caddy routing, or frontend. No government portal was contacted, no OTP/CAPTCHA triggered, no grievance submitted, at any point in this work.

- Date: 2026-09-05
- Request: PR2 of the GovtSync filing simplification — make government FILING universally external (every portal's Escalate action opens the real portal in a new local browser tab, never Needle's remote Playwright session), reusing the existing manual-redirect path, without touching status checking, portal adapters, `backend_govt_live`, Caddy, or the K/M Postgres attempt migration from commit c49371e6.
- Summary: Traced the existing flow first (`resolveEscalateMode()` → `'live'`/`'manual_redirect'`/`'manual_worksheet'`, `handleEscalateManualRedirect()` → `window.open(entryUrl)` + `handlePrepare()`, `/api/govt-portal`'s existing `entry_url`/`live_session_supported`/`live_automation_enabled` fields, `/cases/{id}/govt/translate` and `/cases/{id}/govt/submit` unchanged contracts) and confirmed only `frontend/components/briefcase/BriefcaseCaseModal.jsx` needed to change. Extracted the mode decision into a new pure, exported `resolveGovtEscalateMode(data)` (same pattern as this file's existing `isTamilNaduLiveSessionActive`/`isTamilNaduResolvedPortal` helpers) that now always returns `'manual_redirect'`, deliberately ignoring `live_session_supported`/`live_automation_enabled` for the filing decision — those fields are untouched and still read by the still-live (not retired) `/govt/session/start` endpoint and `browser_session.py` infra, left alone per this PR's scope. `resolveEscalateMode()` now delegates to it, so both existing call sites (`handleEscalateClick`'s ref method and the locked-section "Escalate to proceed" button) automatically always take the existing `handleEscalateManualRedirect()` branch — no second filing mechanism was created. Updated the two now-stale pieces of user-facing/inline copy this decision-collapse made incorrect: the locked-section helper text (dropped the two conditional "no live session"/"automation off" sentences for one unconditional "opens in a new tab" sentence) and the "Reopen {portal}" link, which previously only rendered for portals with `live_session_supported === false` — now renders for every portal with a resolvable URL, since staff on any portal now needs a way back into a closed tab. `handleOpenLive()`/`liveAutomationEnabled`/`portalSupportsLiveSession`/the live-session UI (hosted-sessions panel, capture-reference, close-session) are all left in place, now simply unreachable from Escalate — nothing was deleted.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/tests/briefcase-govt-escalate-mode.test.jsx` (new), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `npx vitest run tests/briefcase-govt-escalate-mode.test.jsx` (10/10, new); `npx vitest run` full frontend suite (33/33, all passing, none weakened); `npm run build --prefix frontend` clean; `venv/bin/python -m pytest tests/test_govt_duplicate_filing.py -q` (12/12, unchanged, zero backend edits); `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (198/198, unchanged); `git diff --stat` confirms zero changes to every protected adapter/status/session file (`rajasthan_sampark.py`, `karnataka_ipgrs.py`, `maharashtra_aaplesarkar.py`, `modules/govt_sync/status/tamil_nadu.py`, `modules/govt_sync/status_attempts.py`, `modules/govt_sync/otp_sessions.py`, `modules/govt_sync/browser_session.py`, `sansadx_backend/db.py`, `admin_api.py`), Caddy, docker-compose, and every backend `.py` file — the only source file changed is the one frontend component.
- Risks or follow-ups: `live_session_supported`/`live_automation_enabled` and the `/govt/session/start`/`/govt/session/start-status-check` live-browser endpoints, `browser_session.py`, `backend_govt_live`, Caddy `@govt_live` routes, and Tamil Nadu's live-browser status implementation are all deliberately untouched and now unreferenced by the filing path — their retirement is explicitly a later, separate cleanup PR, not this one. No government portal was contacted, no OTP/CAPTCHA triggered, no grievance submitted, at any point in this work.

- Date: 2026-09-07
- Request: Standardize the operator-facing "status check completed but nothing meaningful to apply" message across every govt-sync surface (was "Portal check inconclusive — verify manually on the portal.", misleading since it implied leaving Needle) to "No status change detected. Please try again later." — wording only, no logic change, no protected adapter/status file touched.
- Summary: Added one shared constant `_GOVT_NO_STATUS_CHANGE_NOTE` in `api_router.py` and used it at the three exact branches that represent this concept: `govt_poll_case`'s `if not result.checked or not result.status` branch, `govt_status_check_advance`'s (Karnataka/Maharashtra interactive CAPTCHA/OTP) equivalent COMPLETE-but-unrecognized branch, and Tamil Nadu's `govt_tamil_nadu_check_status` fallback (only fires if the protected TN adapter's own per-state `.note` is ever falsy — in practice it always sets a specific message, e.g. "no petition matched this reference number", which is correctly left untouched and NOT replaced). Updated the matching frontend fallback string in `BriefcaseCaseModal.jsx`'s Tamil Nadu check-status handler for consistency. Left every other message alone — needs_verification, CAPTCHA/OTP-required, attempt-expired, portal-unavailable/network-error, and genuine status-check-failure branches keep their existing, distinct wording. The internal `status_check_inconclusive` audit-log action name is unchanged.
- Files touched: `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `tests/test_karnataka_status_check.py`, `tests/test_govt_sync_poller.py`, `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (198/198, same count, updated assertions only, none weakened); `npx vitest run` (33/33); `npm run build --prefix frontend` clean; `git diff --stat` confirms zero changes to `rajasthan_sampark.py`, `karnataka_ipgrs.py`, `maharashtra_aaplesarkar.py`, `modules/govt_sync/status/tamil_nadu.py`, `modules/govt_sync/status/base.py`, `modules/govt_sync/status_attempts.py`, `modules/govt_sync/otp_sessions.py`, `modules/govt_sync/browser_session.py`, `sansadx_backend/db.py`.
- Risks or follow-ups: No new endpoint-level test was added for the Tamil Nadu fallback branch specifically (it has no pre-existing endpoint-level test either, since it's effectively unreachable in production — the protected TN adapter always sets its own `.note`); the two branches that actually matter in practice (the plain poll and the Karnataka/Maharashtra interactive advance, including the exact case from the reported production screenshot) are fully covered by existing, now-updated tests. Investigating the actual raw Karnataka status string that triggered the original screenshot report is separate, deliberately not undertaken here per this task's own scope limits.

- Date: 2026-09-07
- Request: Correct stale Caddy routing (found during the live-browser dependency audit) — move Karnataka/Maharashtra interactive status-check traffic off the single-worker `backend_govt_live` service onto the normal multi-worker `backend`, since commit `c49371e6` already moved their attempt state into PostgreSQL (`govt_status_check_attempts`), removing the single-worker-affinity requirement the Caddy routing was still enforcing.
- Summary: Removed `/api/cases/*/govt/status-check/start` and `/api/cases/*/govt/status-check/*/advance` from `deploy/ec2/Caddyfile`'s `@govt_live` matcher, so they now fall through to the plain `reverse_proxy backend:8000` like every other non-live-browser govt-sync route. Every Tamil Nadu live-browser route (`start-status-check`, `tamil-nadu/check-status`, the `/stream` WebSocket) and the (now UI-unreachable but not-yet-removed) filing session routes remain routed to `backend_govt_live`, untouched. Updated the matcher's own explanatory comment block (previously described the now-obsolete process-local-dict reasoning as if still current) to document the actual, current reasoning and point at `c49371e6`. Updated `scripts/validate_caddy_govt_sync_routing.py`'s two K/M `CASES` entries from expecting `backend_govt_live` to expecting `backend` — this is the same permanent regression script asserting the *new*, correct routing, not a weakened check.
- Files touched: `deploy/ec2/Caddyfile`, `scripts/validate_caddy_govt_sync_routing.py`, `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `python3 scripts/validate_caddy_govt_sync_routing.py` — ALL 13 ROUTING CHECKS PASSED (real Caddy + dummy upstream containers); `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (198/198, unchanged — no adapter/API/frontend code touched); `git diff --stat` confirms zero changes to every adapter (`rajasthan_sampark.py`, `karnataka_ipgrs.py`, `maharashtra_aaplesarkar.py`, `modules/govt_sync/status/tamil_nadu.py`), `status_attempts.py`, `otp_sessions.py`, `browser_session.py`, `sansadx_backend/db.py`, `api_router.py`, `docker-compose.yml`, and the frontend.
- Risks or follow-ups: This is intentionally a routing-only fix. `backend_govt_live`, Playwright/Chromium, `GovtLiveBrowserView`, `/govt/session/start`, `/govt/session/start-status-check`, `live_session_supported`, and `GOVT_LIVE_AUTOMATION_ENABLED` are all deliberately untouched — Tamil Nadu's live status-check still needs all of them, and the filing session routes are orphaned-but-not-yet-removed per the approved dependency audit's removal plan. Not yet pushed or deployed — awaiting separate review/approval before that step.

- Date: 2026-09-07
- Request: Phase 1 of the Tamil Nadu Playwright-to-HTTP migration investigation (Claude Cowork report classification B — "probably migratable, but needs a controlled proof") — build a controlled, read-only diagnostic harness to gather real browser-network evidence and attempt the existing HTTP-replay PoC against an already-authenticated live session, without migrating production TN status checking, without running the live proof yet, and without touching any other government adapter.
- Summary: Added `modules/govt_sync/status/tn_network_diagnostic.py` — a `_NetworkEvidenceCollector` that attaches to a live session's Playwright `BrowserContext` request/response events (redacting URLs to path+param-names, reading only the `content-type` header by name, never touching cookies/Set-Cookie/Authorization/full header dumps), wrapped around an unmodified call to the real, production `TamilNaduStatusAdapter.check_status_on_page()` — no new navigation sequence is added, this observes the exact same click-through the "Check Tamil Nadu status" button already performs today. From the captured evidence it discovers the portal's internal ticket/record id (from an actually-observed `/portal/ta/ticket/{id}` request URL, never guessed or caller-supplied), then attempts the existing, previously-never-executed, untracked `scripts/tn_session_replay_poc.py`'s `tn_session_replay_experiment()` using that real id — now included in this commit since production code has a real import dependency on it. Classifies the outcome into one of `HTTP_SUCCESS` / `AUTH_FAILURE` / `REDIRECT_TO_LOGIN` / `SPA_SHELL` / `API_REQUIRED` / `RECORD_ID_UNRESOLVED` / `UNDETERMINED`, never overstating past what the evidence actually shows. Added one new, disabled-by-default (`GOVT_SYNC_TN_HTTP_DIAGNOSTIC_ENABLED`, default `false`, 404s when off) endpoint in `api_router.py`, `POST /cases/{case_id}/govt/session/{session_id}/tamil-nadu/diagnostic/http-replay-proof`, reusing the existing `_get_tamil_nadu_live_status_context` tenant/case/session-ownership + Tamil-Nadu-adapter check unmodified, plus an extra hard-coded guard refusing any reference number other than grievance `18968314` (the only grievance this entire investigation is authorized against). This endpoint is the only way to reach the diagnostic in production — no standalone-process invocation is possible, since the live session's Playwright state is process-local to whichever single `backend_govt_live` worker opened it (same limitation the PoC's own docstring already documented).
- Files touched: `api_router.py`, `modules/govt_sync/status/tn_network_diagnostic.py` (new), `scripts/tn_session_replay_poc.py` (previously untracked, now committed — a real import dependency of the new module), `tests/test_tamil_nadu_network_diagnostic.py` (new), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `venv/bin/python -m pytest tests/test_tamil_nadu_network_diagnostic.py -q` (25/25, new — pure-function redaction/classification tests, direct network-collector tests with fake Playwright request/response objects, an end-to-end test driving the real `check_status_on_page()` against the existing `tests/test_tamil_nadu_status.py` fakes with simulated network evidence, and endpoint-gating tests for disabled/wrong-reference/enabled paths); `venv/bin/python -m pytest tests/test_tamil_nadu_status.py tests/test_tamil_nadu_network_diagnostic.py -q` (56/56, existing TN adapter tests unmodified); `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (223/223 = the existing 198 baseline + 25 new, confirming zero regression); `git diff --stat` confirms zero changes to every protected adapter (`rajasthan_sampark.py`, `karnataka_ipgrs.py`, `maharashtra_aaplesarkar.py`), `modules/govt_sync/status/tamil_nadu.py` and `status/base.py` (the production TN adapter itself, untouched), `status_attempts.py`, `otp_sessions.py`, `browser_session.py`, `sansadx_backend/db.py`, Caddy, docker-compose, and the frontend.
- Risks or follow-ups: **Caddy routing gap identified, not fixed here (explicitly out of scope, per instruction to STOP and report rather than change it):** the new endpoint's path (`.../tamil-nadu/diagnostic/http-replay-proof`) does not match any existing `@govt_live` pattern in `deploy/ec2/Caddyfile`, so as deployed today it would be routed to the plain multi-worker `backend` — which does NOT hold the live session in memory (that lives only in the single-worker `backend_govt_live` process) — meaning any real invocation would always 404 with "Live session not found" regardless of whether a live session actually exists, until a Caddy pattern for this route is added. This must be resolved (with separate explicit authorization, since Caddy changes are out of this task's scope) before the live proof can actually run. No live proof was run in this task — `capture_tn_diagnostic()` was only exercised against mocks. No production TN status-check behavior changed.

- Date: 2026-09-07
- Request: Fix the Caddy routing gap identified in the previous entry — route the new TN HTTP-diagnostic endpoint (`.../tamil-nadu/diagnostic/http-replay-proof`) to `backend_govt_live`, and nothing else.
- Summary: Added one literal-anchored pattern (`/api/cases/*/govt/session/*/tamil-nadu/diagnostic/http-replay-proof`) to `deploy/ec2/Caddyfile`'s existing `@govt_live` matcher, following the same two-wildcards-with-literal-segments-after-both style already used for every other `govt/session/{id}/...` route on that line — per the matcher's own documented wildcard-collapsing rule, this pattern matches only this exact route. No existing pattern was removed, reordered, or altered. Updated `scripts/validate_caddy_govt_sync_routing.py`'s `CASES` list with 4 new entries: the new diagnostic route (→ `backend_govt_live`), and two explicit Rajasthan OTP-endpoint cases (`/govt/otp/send`, `/govt/otp/verify` → `backend`) that were implied but not directly tested before. Added clarifying comments (not logic changes) noting that the existing Karnataka/Maharashtra status-check cases already cover both states (identical route shape, no portal name in the path).
- Files touched: `deploy/ec2/Caddyfile`, `scripts/validate_caddy_govt_sync_routing.py`, `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `python3 scripts/validate_caddy_govt_sync_routing.py` — ALL 16 ROUTING CHECKS PASSED (real Caddy + dummy upstream containers; 13 previous + 3 new: TN diagnostic route, both Rajasthan OTP endpoints); `venv/bin/python -m pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (223/223, unchanged — no application code touched); `git diff --stat` confirms zero changes to `api_router.py`, `modules/govt_sync/status/tn_network_diagnostic.py`, every protected adapter, `browser_session.py`, `docker-compose.yml`, and the frontend.
- Risks or follow-ups: `GOVT_SYNC_TN_HTTP_DIAGNOSTIC_ENABLED` remains unset/`false` — this change only fixes routing for if/when that flag is explicitly turned on and the live proof is separately authorized; it does not enable anything. Not yet pushed or deployed.

- Date: 2026-09-07
- Request: Implement the design-only proposal from the previous session — a process-local, single-use runtime gate letting the TN HTTP-diagnostic proof be enabled for exactly one controlled run without recreating `backend_govt_live` (which would destroy the in-memory Playwright `LiveSession` the diagnostic needs).
- Summary: Added `modules/govt_sync/status/tn_diagnostic_runtime_gate.py` — an in-memory `dict[session_id -> _Grant]` with `arm()`/`consume_if_armed()`/`disarm()`/`has_pending_grant()`, `asyncio.Lock`-protected, TTL=300s, lazy-expiry (no background sweep), single-use (atomically popped on successful consume). Never imports or touches `modules.govt_sync.browser_session` in any way. Added one new endpoint in `api_router.py`, `POST .../tamil-nadu/diagnostic/arm` (body `{"action": "arm"|"disarm"}`), reusing `_get_tamil_nadu_live_status_context` and the existing hard-coded `18968314` guard unchanged — a grant can never be created for anything the diagnostic endpoint itself wouldn't already accept. Widened the existing diagnostic endpoint's gate from `if not _tn_http_diagnostic_enabled(): 404` to `env flag OR consume_if_armed(...)`, carefully ordered so the pre-existing "flag off → 404 for everyone, no ownership check even attempted" behavior is preserved byte-for-byte for the common case (a cheap `has_pending_grant(session_id)` pre-check runs first; only if a grant might exist does the real ownership/reference/consume path run). `GOVT_SYNC_TN_HTTP_DIAGNOSTIC_ENABLED` itself is completely untouched and keeps its existing meaning. Added the arm/disarm route to `deploy/ec2/Caddyfile`'s `@govt_live` matcher (same single-worker-affinity reasoning as the diagnostic route itself — the grant must live in the same process as the `LiveSession`) and a matching case to `scripts/validate_caddy_govt_sync_routing.py`.
- Files touched: `api_router.py`, `modules/govt_sync/status/tn_diagnostic_runtime_gate.py` (new), `deploy/ec2/Caddyfile`, `scripts/validate_caddy_govt_sync_routing.py`, `tests/test_tamil_nadu_diagnostic_runtime_gate.py` (new), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `pytest tests/test_tamil_nadu_diagnostic_runtime_gate.py tests/test_tamil_nadu_network_diagnostic.py tests/test_tamil_nadu_status.py -q` (80/80 — 24 new gate tests including exact-match consume, tenant/case/session/reference-mismatch fail-closed, TTL expiry, disarm, concurrent-consume-exactly-once, and an explicit mocked+AST-based proof the gate never touches `browser_session`); `pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (247/247 = 223 baseline + 24 new, zero regression); `python3 scripts/validate_caddy_govt_sync_routing.py` — ALL 17 ROUTING CHECKS PASSED (16 previous + 1 new arm route); `git diff --stat` confirms zero changes to `modules/govt_sync/status/tamil_nadu.py`, `tn_network_diagnostic.py`, `browser_session.py`, every protected adapter, `sansadx_backend/db.py`, `docker-compose.yml`, and the frontend.
- Risks or follow-ups: This is still pre-proof — the runtime gate was implemented and tested but never armed, never invoked, and no diagnostic was run. `GOVT_SYNC_TN_HTTP_DIAGNOSTIC_ENABLED` remains unset/`false`. Not pushed, not deployed. Once deployed, using it for the actual controlled proof against Case #3563/grievance 18968314 remains a separate, explicitly-authorized next step.

- Date: 2026-09-07
- Request: Add a case-scoped TN HTTP-diagnostic invocation path so the controlled proof can be run for a known case (`case_id` only) WITHOUT the raw `session_id` ever crossing an HTTP boundary, a request body, a response, a log line, or an exception message — for a real live session (tenant 12, case 3563, grievance 18968314) whose `session_id` was deliberately withheld from this conversation as credential-equivalent material.
- Summary: Added `modules/govt_sync/browser_session.find_live_session_for_case(tenant_id, case_id) -> LiveSession | None` — the one deliberate exception to every other accessor in that module being keyed by `session_id`. Read-only (never mutates/closes a session), exact-match only (both `tenant_id` and `case_id`), and fails closed (returns `None`) if more than one session matches rather than picking arbitrarily. Added `api_router._get_tamil_nadu_live_status_context_for_case(case_id, user)`, a session-id-free sibling of the existing `_get_tamil_nadu_live_status_context`, and one new endpoint, `POST /cases/{case_id}/govt/tamil-nadu/diagnostic/run`, which resolves tenant/case/live-session ownership entirely server-side, enforces the existing `18968314`-only guard, and calls the SAME, unmodified `capture_tn_diagnostic()` the session-ID-based endpoint calls — no new navigation, no new candidate URL, no adapter change. Deliberately bypasses `tn_diagnostic_runtime_gate.py`'s arm/consume mechanism entirely (that gate protects the external, session-ID-guessable flow; here ownership is re-verified fresh end-to-end within one authenticated request, so there's nothing separate to protect and no single-use restriction is needed — this endpoint can be called again after a successful run). The existing session-ID-based `arm`/`http-replay-proof` endpoints are completely unchanged. Added the new route to `deploy/ec2/Caddyfile`'s `@govt_live` matcher (single wildcard, case_id only — needs the same single-worker affinity to see `_sessions`) and a matching case to `scripts/validate_caddy_govt_sync_routing.py`.
- Files touched: `api_router.py`, `modules/govt_sync/browser_session.py`, `deploy/ec2/Caddyfile`, `scripts/validate_caddy_govt_sync_routing.py`, `tests/test_tamil_nadu_diagnostic_case_scoped.py` (new), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `pytest tests/test_tamil_nadu_diagnostic_case_scoped.py -q` (18/18 new — resolver exact-match/cross-tenant/cross-case/fail-closed-on-multiple/read-only-never-closes, endpoint ownership/portal/reference/no-session rejection, diagnostic invocation with the internally-resolved session, proof the gate is never armed/consumed by this path, proof a second call succeeds again (no single-use restriction), and three explicit session_id-never-surfaces tests covering the response body, error responses, and the audit-log payload, plus a static AST-free source-scan proving the resolver never logs); `pytest tests/test_tamil_nadu_diagnostic_case_scoped.py tests/test_tamil_nadu_diagnostic_runtime_gate.py tests/test_tamil_nadu_network_diagnostic.py tests/test_tamil_nadu_status.py -q` (98/98, all existing TN tests unmodified); `pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` (265/265 = 247 baseline + 18 new, zero regression); `python3 scripts/validate_caddy_govt_sync_routing.py` — ALL 18 ROUTING CHECKS PASSED (17 previous + 1 new); `git diff --stat` confirms zero changes to `tamil_nadu.py`, `tn_network_diagnostic.py`, `tn_session_replay_poc.py`, `tn_diagnostic_runtime_gate.py`, every protected adapter, `sansadx_backend/db.py`, `docker-compose.yml`, and the frontend.
- Risks or follow-ups: Still pre-proof — this endpoint was implemented and tested only; it was never invoked, no live session was touched, no government portal was contacted, and the real `session_id` for tenant 12/case 3563 was never retrieved, printed, logged, or exposed anywhere in this work. `GOVT_SYNC_TN_HTTP_DIAGNOSTIC_ENABLED` remains unset/`false` (irrelevant to this new endpoint, which never reads it). Not pushed, not deployed.

- Date: 2026-09-07
- Request: Add a **temporary, disposable** UI button so the operator can click once to invoke the already-deployed case-scoped TN HTTP-diagnostic (`POST /cases/{case_id}/govt/tamil-nadu/diagnostic/run`) without manually running a browser-console `fetch()` snippet — the operator wasn't comfortable doing that by hand. Hard-scoped to case 3563 only; not a general-purpose feature.
- Summary: Added to `frontend/components/briefcase/BriefcaseCaseModal.jsx` (`GovtSyncSection`): a module-level constant `TN_HTTP_DIAGNOSTIC_PROOF_CASE_ID = 3563` and an exported pure gate function `isTnHttpDiagnosticProofCase(caseId)`; local state (`tnHttpDiagnosticBusy`, `tnHttpDiagnosticResult`); a handler `handleRunTnHttpDiagnostic()` that shows a `window.confirm()` with the exact required copy, and only on confirmation calls the existing `apiPost('/api/cases/{caseId}/govt/tamil-nadu/diagnostic/run', {})` — the same authenticated API client every other action in this file already uses, so the JWT never leaves that existing client; and a render block, gated by `isTnHttpDiagnosticProofCase(caseId)`, showing the button ("Run TN HTTP Diagnostic — ONE TIME") and, after a response, the raw JSON of the (already server-sanitized) diagnostic report. No auto-invoke, no retry, no session_id ever read/rendered/logged anywhere in this code (this endpoint doesn't need one). The whole block is explicitly labeled "Temporary — Phase 1 controlled proof only" in both a visible UI banner and code comments, with a one-line note on exactly what to delete once the investigation concludes.
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/tests/briefcase-tn-http-diagnostic-button.test.jsx` (new), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `npx vitest run tests/briefcase-tn-http-diagnostic-button.test.jsx` (13/13 new — the gate function tested directly for case 3563/other ids/null/non-numeric input, plus source-level proofs that the handler calls the real `apiPost` and never a raw `fetch`/manual `Authorization` header/`getAuthToken`, never references `session_id`, never logs to console, requires confirmation strictly before the API call, and is never invoked automatically or retried); `npx vitest run` full frontend suite (46/46, all previously-passing tests unaffected); `npm run build --prefix frontend` clean; `git diff --stat` confirms zero changes to `api_router.py`, the TN adapter, the diagnostic harness, the runtime gate module, `browser_session.py`, every protected backend adapter, Caddy, and Docker config — this is a frontend-only change.
- Risks or follow-ups: **This is deliberately temporary and must be removed** once the controlled proof is complete and reviewed — delete the `TN_HTTP_DIAGNOSTIC_PROOF_CASE_ID` constant, `isTnHttpDiagnosticProofCase()`, `handleRunTnHttpDiagnostic()`, its state, its render block, and this test file together as one follow-up commit. The button was not clicked, the endpoint was not invoked, no government portal was contacted, and no grievance was created or modified during this implementation. Not pushed, not deployed, per instruction.

- Date: 2026-09-08
- Request: Frontend-cleanup-only task — remove the temporary "Run TN HTTP Diagnostic — ONE TIME" button (commit `2909b619`) from the shipped frontend now that the controlled Phase 1/Phase 2 TN diagnostic evidence-gathering is complete. Explicitly do not touch the backend Phase-1/Phase-2 diagnostic code, the case-scoped endpoint, or any adapter.
- Summary: Removed from `frontend/components/briefcase/BriefcaseCaseModal.jsx` exactly the block the prior entry flagged for follow-up removal: the `TN_HTTP_DIAGNOSTIC_PROOF_CASE_ID` constant, the exported `isTnHttpDiagnosticProofCase(caseId)` gate function, the `tnHttpDiagnosticBusy`/`tnHttpDiagnosticResult` state, the `handleRunTnHttpDiagnostic()` handler (its `window.confirm()` flow and its `apiPost('/api/cases/{caseId}/govt/tamil-nadu/diagnostic/run', {})` call), and the "Temporary — Phase 1 controlled proof only" banner + button + result `<pre>` render block. Deleted the now-fully-obsolete `frontend/tests/briefcase-tn-http-diagnostic-button.test.jsx` (it existed solely to test the removed code — its only reference to `isTnHttpDiagnosticProofCase` was the removed export). No replacement UI, no new route, no other code in `BriefcaseCaseModal.jsx` touched (the shared `Loader2` import stays — used by 16 other unrelated busy-state spinners in the same file).
- Files touched: `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `frontend/tests/briefcase-tn-http-diagnostic-button.test.jsx` (deleted), `TASK_LOG.md`, `PROJECT_MEMORY.md`
- Verification: `npx vitest run` full frontend suite — all previously-passing tests still pass, one fewer test file (the deleted one); `npm run build --prefix frontend` clean; `pytest tests/ -k "govt or karnataka or maharashtra or rajasthan or tamil" -q` unaffected (frontend-only change, backend untouched); `git diff --stat` confirms zero changes to `api_router.py`, `tn_network_diagnostic.py`, `tn_http_replay_diagnostic.py`, `tn_diagnostic_runtime_gate.py`, `tamil_nadu.py`, `browser_session.py`, every protected adapter, `otp_sessions.py`, Caddy, and any database/schema file.
- Risks or follow-ups: None — the backend Phase-1 case-scoped diagnostic endpoint (`POST /cases/{case_id}/govt/tamil-nadu/diagnostic/run`) and the Phase-2 multi-ticket/durability harness remain fully intact and deployed; only the temporary frontend trigger is gone. No push, no deploy, no live TN session opened or touched, no grievance created or modified, no government portal contacted.

- Date: 2026-09-08
- Request: Implement Tamil Nadu inboard HTTP status checking with dedicated Fernet-encrypted cookie-session persistence, while retaining Playwright as the human login/re-authentication fallback and avoiding any live TN portal request/deploy.
- Summary: Added `govt_cookie_sessions` as a tenant/portal-scoped encrypted cookie-session store using a single `GOVT_COOKIE_SESSION_KEY` Fernet key (no JWT_SECRET reuse, no MultiFernet in v1). Added `modules/govt_sync/cookie_sessions.py` for encrypt/decrypt/load/store/lifecycle state, and `modules/govt_sync/adapters/tamil_nadu_http.py` as the primary inboard read-only TN status adapter. Registered `status_check_adapter: "tamil_nadu_http_api"` for Tamil Nadu and exposed safe cookie verification state through `/api/govt-portal`. Added a Playwright-session promotion endpoint that reuses the existing read-only TN status reader after staff login, observes one naturally requested `/portal/api/tickets/{record_id}` URL, and persists only encrypted cookies plus full-reference -> record-id mapping. Added the promotion route to `deploy/ec2/Caddyfile`'s `@govt_live` matcher plus the validator case because the endpoint must read the process-local LiveSession in `backend_govt_live`. Briefcase now shows normal `Check status now` when TN access is verified, otherwise prompts staff to verify on the Tamil Nadu portal; no cookies, internal record ids, session ids, API URLs, JWTs, or raw errors are displayed.
- Files touched: `.env.example`, `api_router.py`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `main.py`, `modules/data/govt_portals.json`, `modules/govt_sync/adapters/__init__.py`, `modules/govt_sync/adapters/tamil_nadu_http.py`, `modules/govt_sync/cookie_sessions.py`, `modules/govt_sync/status/tamil_nadu.py`, `sansadx_backend/db.py`, `deploy/ec2/Caddyfile`, `deploy/ec2/env.production.example`, `scripts/validate_caddy_govt_sync_routing.py`, `tests/test_tamil_nadu_inboard_status.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Verification: `venv/bin/python -m pytest tests/test_tamil_nadu_inboard_status.py -q` passed (14/14); `venv/bin/python -m pytest tests/test_tamil_nadu_inboard_status.py tests/test_tamil_nadu_status.py tests/test_tamil_nadu_network_diagnostic.py tests/test_tamil_nadu_http_replay_diagnostic.py tests/test_tamil_nadu_http_replay_diagnostic_endpoint.py tests/test_tamil_nadu_diagnostic_runtime_gate.py tests/test_tamil_nadu_diagnostic_case_scoped.py tests/test_karnataka_status_check.py tests/test_maharashtra_status_check.py tests/test_govt_otp_audit.py tests/test_govt_status_normalization.py tests/test_govt_status_manual_window.py tests/test_govt_portals_config.py tests/test_govt_sync_poller.py tests/test_govt_sync_poll_lock.py tests/test_govt_duplicate_filing.py -q` passed (282/282); `venv/bin/python -m py_compile ...` passed for touched Python files plus the Caddy validator; `npm run build --prefix frontend` passed; `npm run test --prefix frontend` passed (7 files / 33 tests). `python3 scripts/validate_caddy_govt_sync_routing.py` extracted the updated matcher but could not complete because local Docker failed creating the validation network.
- Risks or follow-ups: Requires `GOVT_COOKIE_SESSION_KEY` to be configured in production before TN cookie promotion/status checks can work; changing the single key invalidates stored TN sessions and staff must re-authenticate. This implementation was tested with mocked HTTP/Playwright only; no live TN request was made, no grievance was created/modified, no commit/push/deploy occurred.

# 2026-09-18 — Case Detail approved UX/UI refinement

- User approved the complete preview palette and workflow/accessibility recommendations, then explicitly confirmed repository edits. Local implementation only; no push, deployment, live WhatsApp send, or government-portal action.
- Applied parchment/charcoal/forest-green/amber tokens, rem typography, focus/press feedback and reduced-motion/transparency support. Added accessible complaint tabs and disclosures, truthful complaint scope, corrected government-status handler ID, mobile next-action placement, and preserved the citizen-first independent-lifecycle workspace.
- Extracted `CitizenReplyDialog.jsx` and scoped `case-detail.css`; reply preview is editable, errors retain drafts, ordinary sends require no typed case reference, and internal notes are independent. Backend notification no longer auto-resolves; explicit resolution confirms complaint scope and retains the workspace through `useBriefcaseCases`.
- Validation: production frontend build and 53 Vitest tests pass. Desktop/mobile Playwright checks cover reply without resolution, explicit resolution, retained workspace, labelled controls and reply focus. Backend suite: 25 pass, four unchanged case-list tests fail because SQLite cannot execute the existing PostgreSQL `SELECT DISTINCT ON` query at `api_router.py:1939`. Run test fixtures with `TZ=UTC` to avoid their naive-UTC `iat` being interpreted in the host timezone. Network sends are mocked.
...
# 2026-09-19 — Government Portal History implementation

- Request: Implement the approved, tightly scoped Government Portal History architecture without live Rajasthan history retrieval, document downloading, citizen automation, automatic closure, or deployment/push.
- Summary: Added generic history capability/availability states and normalized event/document models; a dedicated immutable `govt_portal_history_events` store with database-arbitrated deduplication; shared identity utilities that preserve the exact legacy snapshot identity contract; `GovtSyncChangeSet` meaningful-change signals; the tenant/case/portal/reference-scoped `GET /api/cases/{case_id}/govt/portal-history`; and terminal portal-event IDs in resolution-review context without changing `cases.status`. Added a pure local-fixture Rajasthan mapper while leaving the adapter `SECURITY_BLOCKED` on its inherited zero-I/O history implementation. Case Detail now renders a separate newest-first Government Portal History timeline only when events exist, initially shows three entries, and makes the latest terminal government remark available in the existing explicit Resolution Review flow. Existing Full History remains the Needle status-check timeline.
- Files touched: `api_router.py`, `main.py`, `sansadx_backend/db.py`, `modules/govt_sync/{identity.py,portal_history.py,orchestrator.py,poller.py,status_snapshot.py}`, `modules/govt_sync/adapters/{__init__.py,base.py,rajasthan_sampark.py,rajasthan_history_mapping.py}`, `frontend/components/briefcase/BriefcaseCaseModal.jsx`, `tests/test_govt_portal_history.py`, `tests/fixtures/rajasthan_history.json`, `frontend/tests/briefcase-govt-portal-history.test.jsx`, `PROJECT_MEMORY.md`, and `TASK_LOG.md`.
- Schema migration: startup migration creates `govt_portal_history_events` idempotently with a unique `(tenant_id, case_id, portal_id, reference_number, event_identity)` constraint and scoped read indexes. Clean-schema creation, existing-schema reapplication, preserved row data, constraint, and index presence are covered by regression tests.
- Verification: focused backend history/snapshot/poller/normalization suite passed (69/69); final portal-history suite passed (19/19); broader relevant backend suite passed (465 passed, 487 deselected, 10 pre-existing deprecation warnings); full frontend Vitest suite passed (10 files / 68 tests); production frontend build passed, including Next lint/type checks (only existing environment warnings for WASM SWC fallback, multiple lockfiles, and stale Browserslist data); `git diff --check` passed. The UI anti-pattern detector reported one existing `borderLeft: 3px` warning in the unchanged Full History component, which was intentionally not altered.
- Safety review: the Rajasthan fixture mapper contains no URL, network, browser, or fetch code; the adapter inherits the base zero-I/O `fetch_history()` and has a regression test asserting no `requests.post` call while security-blocked. Its existing `_fetch_status()` path remains unchanged and is characterized by a regression test. Public history responses omit `source_metadata` and raw portal payloads; exact four-key isolation is present on persistence, pagination cursors, timeline reads, and terminal-remark reads. Legacy `persist_successful_status_result()` and legacy audit-payload shape remain compatible when no history attempt occurs.
- Risks or follow-ups: no production adapter is `SUPPORTED` yet, so the timeline remains dormant until a separately reviewed adapter is authorized. Rajasthan live history and document retrieval remain deliberately unavailable. Migration behavior was validated against isolated local PostgreSQL 15 as recorded below; no production database, portal, network retrieval, push, deploy, or local commit was performed.

## 2026-09-19 — Government Portal History final pre-commit validation

- Real PostgreSQL validation: used the repository's already-running local PostgreSQL 15 container and an isolated disposable database, then removed that database. Extracted the exact three history DDL statements from `main.py`. Verified the real clean-start order (ORM table creation followed by startup DDL), all 28 columns, JSONB document/metadata fields, unique constraint, descending case timeline index, portal/reference index, idempotent second run, existing-database upgrade, survival of pre-existing case/marker/history data, sequential `ON CONFLICT` deduplication, immutable original comment text, and an eight-worker concurrent duplicate race resulting in exactly one row.
- Review fixes: aligned the ORM with PostgreSQL startup DDL using PostgreSQL JSONB variants and a descending timeline index; removed document-level `source_metadata` from the public API projection; added a non-leakage regression test; added equal-date/null-date keyset pagination coverage; and hardened the timeline with a semantic heading and 44px expansion-control target.
- Final checks: relevant backend suite 465 passed / 487 deselected; portal-history suite 19 passed; frontend 10 files / 68 tests passed; production frontend build passed; `git diff --check` passed. Rajasthan remains `SECURITY_BLOCKED`, inherits the zero-I/O history method, and its existing OTP/status methods are unchanged. No live portal call, production access, citizen message, automatic case closure, commit, push, or deployment occurred.
