...
- Files touched: `main.py`, `tests/test_e2e_core_flow.py`, `PROJECT_MEMORY.md`, `TASK_LOG.md`
- Risks or follow-ups: Not yet deployed — deploy pending. All three phases (A: tenant-scoped heuristics, B: AI multilingual fallback, C: contextless media attachment) of the CASE_COMMENT system are now built; recommend a short observation window on `case_comment`/`media_evidence_attached` event volume before deciding on the Briefcase badge (deliberately deferred in the original design discussion). The pre-existing full-`tests/`-directory cross-file pollution (37 unrelated failures) is worth a separate cleanup pass — likely needs either per-file pytest invocation in CI or fixing shared global state (`main.engine`, `dbmod.SessionLocal`) across test files that don't expect to share a process.
