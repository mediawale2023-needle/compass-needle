...
- Date: 2026-09-15
- Request: Implement first readiness-audit fix: tenant-history isolation and legacy notification authorization.
- Changes: Required tenant_id in get_user_context and passed current_tenant from enrichment; scoped both SQL paths and excluded deleted cases; applied existing primary-account check to legacy notify before case lookup. Added API regression tests for staff rejection, cross-tenant rejection, owner/MP access and shared-phone history isolation; updated clarification mocks.
- Verification: Isolated executions of exact source functions passed tenant/deletion isolation and authorization-before-query checks; original source reproduced history leak. Python compilation and git diff --check passed. Full pytest/API suite could not run: pytest/SQLAlchemy absent and package downloads timed out. PostgreSQL not exercised.
- Delivery: Prepared on Codex/tenant-history-notify-auth-20260915 from main 741e6c6 for draft PR review. No merge or deployment.

- GitHub delivery: pushed implementation 7dddfdbc4e6c9b4e27267730315d48f8f488916e to Codex/tenant-history-notify-auth-20260915 and opened draft PR #135 on 2026-09-15. This delivery record is a documentation-only follow-up on the same branch. Not merged or deployed; full API tests remain pending.
