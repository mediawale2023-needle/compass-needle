...
This module is intentionally self-contained: it has no import of, and no
runtime dependency on, any Tamil Nadu filing/submission code. The small
reference/checkpoint helpers below are inlined copies of the same logic (not
an import) so this read-only status reader can exist without a filing
...
# ── Inlined reference/checkpoint helpers (no filing/ dependency) ───────────
# These are read-only string helpers: recognizing a Tamil Nadu reference
# number's shape and noticing that a page is asking a human to sign
# in/OTP/CAPTCHA. They never submit, click, or mutate anything themselves.
