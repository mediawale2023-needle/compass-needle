...
(the same infrastructure modules.govt_sync.filing uses) for portals whose
status page is only reachable behind a real human login — currently the
Tamil Nadu CM Helpline. It is strictly read-only:
...
class StatusCheckState:
    ...
    #    an inconclusive check, never shown as success. AUTH/OTP/CAPTCHA reuse
    #    the same words as modules.govt_sync.filing.base.FilingState.
