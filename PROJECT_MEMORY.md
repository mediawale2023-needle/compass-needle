...
- The admin API now includes a protected tenant-specific synthetic grievance seeder at `/api/admin/seed-constituency-cases`. It is capped at 500 cases per call, tags every inserted case via `case_metadata.synthetic_seed`, clears only prior synthetic seeded cases for that tenant when requested, and should prefer shared seat geography localities/assemblies over hardcoded fallback places so demos stay constituency-aware.
